...
export class TripPlannerService {
  ...
  async generatePlans(input: PlanTripDto): Promise<PlanTripResult> {
    ...
    if (rulePlans.length === 0) {
      return {
        ...
        source: 'rule',
        warning:
          'AI 暂时不可用，且未在圣地库中匹配到与您诉求高度相关的目的地。请尝试在标题或备注中提及具体的文化传统/祖师/圣地名称，或在下方手动选择圣地。',
      };
    }
    return {
      plans: rulePlans,
  ...
  private async extractMentionedSites(
    ...
    const systemPrompt = `你是一个文本信息提取器。任务：从用户的旅行需求文本中,提取所有明确提到的具体祖庭/寺院/道观/教堂/清真寺/神社/文化圣地名称。
...
religionGuess 可选值: 佛教/道教/禅宗/基督教/天主教/伊斯兰教/印度教/犹太教/儒教/锡克教/神道教/藏传佛教/巴哈伊教/原住民/未知`;
  ...
  private async enrichMissingSites(
    ...
    const systemPrompt = `你是一位精通全球文化历史地理的专家。任务:为给定的祖庭/寺院/道观/教堂/清真寺名称生成结构化资料。
...
4. description 100-200 字中文,介绍历史背景和文化地位
...
{"sites":[{"name":"纯阳观","nameEn":"Chunyang Temple","country":"中国","city":"广州","latitude":23.1295,"longitude":113.2644,"religionSlug":"taoism","description":"广州纯阳观位于...","confidence":0.92,"unknown":false}]}`;
  ...
  private async callLLMForPlans(
    ...
    const candidatesText = candidates
      .map(
        (c) =>
          `- id="${c.id}" 名称="${c.name}" 国家="${c.country}" 文化传统="${c.religionName}" 简介="${c.description}"`,
      )
      .join('\n');

    const systemPrompt = `你是佳绩之旅平台的专业行程规划师，精通 12 大文化传统（佛教文化/道教文化/禅宗/基督文化/伊斯兰文化/印度文化/犹太文化/儒家文化/锡克文化/神道文化/藏传文化/巴哈伊文化等）。

你的任务：根据用户的诉求，从我提供的候选圣地数据库中**精选相关的圣地**，输出 2-3 条不同主题/节奏的路线方案。

**铁律（按优先级排列）**：
1. **用户备注是最高优先级！** 备注中描述的主题、线索、时间轴、路线要求、历史脉络等，是规划方案的核心依据。你必须深入理解备注含义并据此设计路线逻辑。例如"基于六祖大师生平弘法时间轴来"意味着必须按慧能大师出生(新州/国恩寺)→求法(东山寺/黄梅)→剃度/弘法(光孝寺/南华寺)的历史时间顺序来编排圣地，而不是随机拼凑。**每个圣地都必须与用户指定的主题人物或事件有直接的历史关联**，不能仅因为同属一个文化传统就混入无关圣地（例如少林寺与六祖慧能无关，不应出现在慧能路线中）
...
5. **如果用户在备注中明确列出了一组文化探访地点(无论是 DB 内还是 ext_),至少有一条方案必须把用户列出的全部地点串联起来,作为"用户指定路线"方案**
    ...
    if (classicCount >= 2) {
      plans.push({
        ...
        subtitle: fallbackPool
          ...
          .map((s) => s.name)
          .join(' → '),
        summary: `精选 ${classicCount} 处核心圣地，节奏适中，适合首次文化探访`,
        days,
    ...
    if (deepCount >= 4 && fallbackPool.length >= 4) {
      plans.push({
        ...
        subtitle: fallbackPool
          ...
          .map((s) => s.name)
          .join(' → '),
        summary: `${deepCount} 处圣地深度走访，每处停留充分，适合资深文化旅行者`,
        days: days + 2,
    ...
    if (quickCount >= 2) {
      plans.push({
        ...
        subtitle: fallbackPool
          ...
          .map((s) => s.name)
          .join(' → '),
        summary: `${quickCount} 处必访圣地紧凑行程，适合时间有限的文化旅行者`,
