...
import {
  fetchPgThemes,
  fetchPgCases,
  type PersonalGrowthTheme,
  type PersonalGrowthCase,
...
export default function PersonalGrowthLanding() {
  ...
  useEffect(() => {
    Promise.all([fetchPgThemes(1, 12), fetchPgCases(1, 6)])
      .then(([t, c]) => {
        setThemes(t.items || []);
        setCases(c.items || []);
      })
  ...
  return (
    <>
      <Stack.Screen options={{ title: '个人圆满' }} />
      <ScrollView style={s.container}>
        <LinearGradient colors={['#1A1200', '#3D2E12']} style={s.hero}>
          <Text style={s.heroKicker}>M34 · PERSONAL GROWTH</Text>
          <Text style={s.heroTitle}>
            个人<Text style={{ color: GOLD_LIGHT }}>圆满</Text>
          </Text>
        ...
        <View style={s.section}>
          ...
          <Text style={s.sectionTitle}>修行六境</Text>
          <Text style={s.sectionSub}>每一主题 · 一次深度蜕变</Text>
          <View style={s.grid}>
            {themes.map((t) => (
              <Pressable
                key={t.id}
                style={[s.themeCard, { borderLeftColor: t.color || GOLD }]}
                onPress={() => router.push(`/personal-growth/themes/${t.slug}` as never)}
              >
                {t.coverUrl ? (
                  <Image source={{ uri: t.coverUrl }} style={s.themeCover} resizeMode="cover" />
                ) : (
                  <View style={[s.themeCover, { backgroundColor: t.color || GOLD, alignItems: 'center', justifyContent: 'center' }]}>
                ...
                <View style={s.themeBody}>
                  ...
                  <View style={s.themeMeta}>
                    ...
                    {t.priceFrom ? (
                      <Text style={[s.metaItem, { color: GOLD }]}>￥{t.priceFrom.toLocaleString()}起</Text>
                    ) : null}
                  </View>
                </View>
              </Pressable>
        ...
        {cases.length > 0 && (
          <View style={s.section}>
            <Text style={s.sectionKicker}>蜕变案例</Text>
            <Text style={s.sectionTitle}>他们这样重生</Text>
            <Pressable style={s.seeAll} onPress={() => router.push('/personal-growth' as never)}>
              <Text style={s.seeAllText}>全部案例 →</Text>
            </Pressable>
            {cases.map((c) => (
              <Pressable
                key={c.id}
                style={s.caseCard}
                onPress={() => router.push(`/personal-growth/cases/${c.slug}` as never)}
              >
                <View style={s.caseHead}>
                  ...
                  {c.theme && (
                    <View style={[s.caseTag, { backgroundColor: (c.theme.color || GOLD) + '22' }]}>
                      <Text style={[s.caseTagText, { color: c.theme.color || GOLD }]}>{c.theme.title}</Text>
                    </View>
                  )}
                </View>
...
const s = StyleSheet.create({
  ...
  heroKicker: { fontSize: 11, color: GOLD_LIGHT, letterSpacing: 2, fontWeight: '700', marginBottom: 10 },
  ...
  themeSub: { fontSize: 12, color: GOLD, marginTop: 2, letterSpacing: 1 },
  ...
  seeAll: { marginBottom: spacing.md },
  seeAllText: { color: GOLD, fontSize: 13, fontWeight: '600' },
  caseCard: { backgroundColor: '#fefbf5', padding: spacing.md, borderRadius: borderRadius.md, borderWidth: 1, borderColor: '#f3e8d0', marginBottom: spacing.md },
  ...
  caseBadgeText: { fontSize: 11, color: GOLD, fontWeight: '700' },
  ...
  highlightChip: { paddingHorizontal: 8, paddingVertical: 3, backgroundColor: '#fff', borderRadius: borderRadius.sm, borderWidth: 1, borderColor: '#f3e8d0' },
  highlightText: { fontSize: 11, color: GOLD },
});
