...
interface Theme {
  ...
  richContent?: {
    ...
    testimonials?: Array<{ name: string; role?: string; before?: string; after?: string; quote?: string }>;
  };
}

export default function FamilyHarmonyDetail() {
  ...
  useEffect(() => {
    if (!slug) return;
    fetch(`${API_BASE}/api/family-harmony/themes/${slug}`)
      .then((r) => r.json())
      .then(setData)
      .catch(() => setData(null))
  ...
  if (!data) return <div className="p-8 text-center text-gray-500">未找到<Link to="/family-harmony" className="text-[#3264ff] ml-2">返回</Link></div>;

  const color = data.color || "#2D8B6F";
      ...
      <div className="max-w-2xl mx-auto px-4 py-4 space-y-4">
        ...
        {data.rituals && data.rituals.length > 0 && (
          <section className="bg-white rounded-xl p-4">
            <h3 className="font-semibold text-base mb-3">🌸 仪轨</h3>
        ...
        {data.richContent?.testimonials && data.richContent.testimonials.length > 0 && (
          <section className="bg-white rounded-xl p-4">
            <h3 className="font-semibold text-base mb-3">💬 家人见证</h3>
