...
export default function PersonalGrowth() {
  ...
  useEffect(() => {
    Promise.all([fetchPgThemes(1, 12).catch(() => null), fetchPgCases(1, 12).catch(() => null)])
      .then(([th, cs]) => {
        setThemes(th?.items ?? []);
        setCases(cs?.items ?? []);
      })
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-10">
      ...
      <div
        ...
        style={{ background: `linear-gradient(135deg, #1A1200 0%, #3D2E12 100%)` }}
      >
        <p className="text-[11px] font-bold tracking-[2px]" style={{ color: GOLD_LIGHT }}>
          M34 · PERSONAL GROWTH
        </p>
        <h2 className="text-2xl font-bold text-white mt-2">
          个人<span style={{ color: GOLD_LIGHT }}>圆满</span>
        </h2>
        <p className="text-xs text-white/80 mt-2 leading-relaxed">
          企业家的内修六境<br />觉醒 · 定力 · 格局 · 重生 · 慈悲 · 传灯
        </p>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-4 py-4 space-y-3">
        {loading ? (
              ...
              >
                ...
                <div className="p-4 flex items-start gap-3">
                  <div
                    ...
                    style={{ backgroundColor: `${th.color || GOLD}20` }}
                  >
                    {th.icon || "🌅"}
                  </div>
          ...
          cases.map((c) => (
            <Link
              key={c.id}
              to={`/personal-growth/cases/${c.slug}`}
              className="block bg-[#fefbf5] border border-[#f3e8d0] rounded-xl p-4 hover:shadow-md transition-shadow"
            >
              <div className="flex justify-between items-center mb-2">
                ...
                {c.theme && (
                  <span
                    ...
                    style={{ backgroundColor: `${c.theme.color || GOLD}22`, color: c.theme.color || GOLD }}
                  >
                    {c.theme.title}
                  </span>
                )}
              ...
              {c.highlights && c.highlights.length > 0 && (
                <div className="flex flex-wrap gap-1.5 mt-2.5">
                  {c.highlights.slice(0, 3).map((h, i) => (
                    <span
                      key={i}
                      className="px-2 py-0.5 bg-white border border-[#f3e8d0] rounded text-[11px]"
                      style={{ color: GOLD }}
                    >
                      ✓ {h}
                    </span>
