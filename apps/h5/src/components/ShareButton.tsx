...
export default function ShareButton({
  ...
  const handleChannelShare = async (platform: string) => {
    ...
    switch (platform) {
      case "WECHAT":
        setShowQR(true); break;
      case "WEIBO":
        window.open(`https://service.weibo.com/share/share.php?url=${encodedUrl}&title=${text}`, "_blank", "width=600,height=500"); break;
      case "FACEBOOK":
        window.open(`https://www.facebook.com/sharer/sharer.php?u=${encodedUrl}`, "_blank", "width=600,height=500"); break;
      case "TWITTER":
        window.open(`https://twitter.com/intent/tweet?text=${text}&url=${encodedUrl}`, "_blank", "width=600,height=500"); break;
      case "WHATSAPP":
        window.open(`https://api.whatsapp.com/send?text=${text}%20${encodedUrl}`, "_blank"); break;
      case "COPY":
  ...
  return (
    <div ref={ref} className={`relative inline-block ${className}`}>
      <button
        ...
        className="flex items-center gap-1.5 px-3 py-1.5 text-sm text-gray-600 hover:text-[#3264ff] hover:bg-blue-50 rounded-lg transition-colors"
