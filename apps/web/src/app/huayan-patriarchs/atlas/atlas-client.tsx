...
const CONFIG: AtlasConfig = {
  religionKey: "huayan", title: "华严宗祖师大图谱", subtitle: "法界缘起 · 一即一切", themeColor: "#C4A265",
  backUrl: "/huayan-patriarchs", detailUrlPrefix: "/huayan-patriarchs", defaultCenter: [34, 109], defaultZoom: 6,
  filters: [{ key: "all", name: "全部", color: "#C4A265" }],
  legendItems: [{ name: "华严宗", color: "#C4A265" }],
  schoolColors: SCHOOL_COLORS, typeLabels: DEFAULT_TYPE_LABELS, loadingText: "正在加载华严宗祖师大图谱...",
};
...
export default function AtlasClient({ patriarchs }: { patriarchs: Patriarch[] }) {
  ...
  return (
    <div className="fixed inset-0 bg-[#0f172a]">
      ...
      {!selectedId && <AtlasLegend title="华严宗" items={CONFIG.legendItems} themeColor={CONFIG.themeColor} />}
      ...
      {!selectedId && (
        <div className="absolute bottom-6 right-4 z-[999] text-right pointer-events-none">
          <h1 className="text-2xl md:text-3xl font-bold text-[#C4A265]/80 tracking-wider" style={{ fontFamily: "'Noto Serif SC', serif" }}>华严宗祖师大图谱</h1>
          <p className="text-xs text-slate-500 mt-1">{patriarchs.length}位祖师 · 法界缘起 · 一即一切</p>
        </div>
      )}
    </div>
  );
