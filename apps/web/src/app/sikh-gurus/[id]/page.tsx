...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  try {
    ...
    return {
      title: `${p.name}${p.title ? " · " + p.title : ""} — 锡克教文化古鲁 | JOINUS`,
      description: p.biography?.slice(0, 160),
      openGraph: { title: `${p.name} — 锡克教文化古鲁`, description: p.biography?.slice(0, 160), ...(p.imageUrl ? { images: [p.imageUrl] } : {}) },
    };
  } catch { return { title: "锡克教文化古鲁 | JOINUS" }; }
}
    ...
    return (
      <main className="min-h-screen bg-[#FFF7ED] flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/sikh-gurus" className="px-6 py-3 bg-[#EA580C] hover:bg-[#C2410C] text-white font-medium rounded-xl transition-colors text-sm">返回锡克教文化古鲁</Link>
        </div>
      </main>
    );
  }
