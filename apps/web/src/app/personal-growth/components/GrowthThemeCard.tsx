...
export default function GrowthThemeCard({ theme }: { theme: PersonalGrowthTheme }) {
  ...
  return (
    <Link
      ...
      className="group block rounded-2xl overflow-hidden bg-white border border-gray-100 shadow-sm hover:shadow-xl hover:border-[#8B6914]/30 hover:-translate-y-1 transition-all"
    >
      <div
      ...
      <div className="p-5">
        {theme.subtitle && (
          <p className="text-gray-500 text-sm mb-3 line-clamp-2">{theme.subtitle}</p>
        )}
        <div className="flex flex-wrap gap-2 mb-4">
          {theme.keywords.slice(0, 3).map((k) => (
            <span
              key={k}
              className="px-2.5 py-1 text-xs rounded-full bg-amber-50 text-[#8B6914] font-medium border border-amber-200"
            >
              {k}
            </span>
          ))}
