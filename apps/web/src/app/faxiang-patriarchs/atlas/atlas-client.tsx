...
const CONFIG: AtlasConfig = {
  religionKey: "faxiang", title: "法相宗祖师大图谱", subtitle: "丝绸之路 · 唯识万法", themeColor: "#C4A265",
  backUrl: "/faxiang-patriarchs", detailUrlPrefix: "/faxiang-patriarchs", defaultCenter: [30, 100], defaultZoom: 4,
  filters: [{ key: "all", name: "全部", color: "#C4A265" }],
  legendItems: [{ name: "法相宗", color: "#C4A265" }],
  schoolColors: SCHOOL_COLORS, typeLabels: DEFAULT_TYPE_LABELS, loadingText: "正在加载法相宗祖师大图谱...",
};
...
export default function AtlasClient({ patriarchs }: { patriarchs: Patriarch[] }) {
  ...
  return (
    <div className="fixed inset-0 bg-[#0f172a]">
      ...
      {!selectedId && <AtlasLegend title="法相宗" items={CONFIG.legendItems} themeColor={CONFIG.themeColor} />}
      ...
      {!selectedId && (
        <div className="absolute bottom-6 right-4 z-[999] text-right pointer-events-none">
          <h1 className="text-2xl md:text-3xl font-bold text-[#C4A265]/80 tracking-wider" style={{ fontFamily: "'Noto Serif SC', serif" }}>法相宗祖师大图谱</h1>
          <p className="text-xs text-slate-500 mt-1">{patriarchs.length}位祖师 · 丝绸之路 · 唯识万法</p>
        </div>
      )}
    </div>
  );
