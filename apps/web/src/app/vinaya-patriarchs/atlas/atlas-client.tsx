...
const CONFIG: AtlasConfig = {
  religionKey: "vinaya", title: "律宗祖师大图谱", subtitle: "南山律 · 东渡传戒", themeColor: "#C4A265",
  backUrl: "/vinaya-patriarchs", detailUrlPrefix: "/vinaya-patriarchs", defaultCenter: [34, 109], defaultZoom: 5,
  filters: [{ key: "all", name: "全部", color: "#C4A265" }],
  legendItems: [{ name: "律宗", color: "#C4A265" }],
  schoolColors: SCHOOL_COLORS, typeLabels: DEFAULT_TYPE_LABELS, loadingText: "正在加载律宗祖师大图谱...",
};
...
export default function AtlasClient({ patriarchs }: { patriarchs: Patriarch[] }) {
  ...
  return (
    <div className="fixed inset-0 bg-[#0f172a]">
      ...
      {!selectedId && <AtlasLegend title="律宗" items={CONFIG.legendItems} themeColor={CONFIG.themeColor} />}
      ...
      {!selectedId && (
        <div className="absolute bottom-6 right-4 z-[999] text-right pointer-events-none">
          <h1 className="text-2xl md:text-3xl font-bold text-[#C4A265]/80 tracking-wider" style={{ fontFamily: "'Noto Serif SC', serif" }}>律宗祖师大图谱</h1>
          <p className="text-xs text-slate-500 mt-1">{patriarchs.length}位祖师 · 南山律宗 · 东渡传戒</p>
        </div>
      )}
    </div>
  );
