...
const CONFIG: AtlasConfig = {
  religionKey: "sanlun", title: "三论宗祖师大图谱", subtitle: "丝绸之路 · 中观空义", themeColor: "#C4A265",
  backUrl: "/sanlun-patriarchs", detailUrlPrefix: "/sanlun-patriarchs", defaultCenter: [35, 100], defaultZoom: 4,
  filters: [{ key: "all", name: "全部", color: "#C4A265" }],
  legendItems: [{ name: "三论宗", color: "#C4A265" }],
  schoolColors: SCHOOL_COLORS, typeLabels: DEFAULT_TYPE_LABELS, loadingText: "正在加载三论宗祖师大图谱...",
};
...
export default function AtlasClient({ patriarchs }: { patriarchs: Patriarch[] }) {
  ...
  return (
    <div className="fixed inset-0 bg-[#0f172a]">
      ...
      {!selectedId && <AtlasLegend title="三论宗" items={CONFIG.legendItems} themeColor={CONFIG.themeColor} />}
      ...
      {!selectedId && (
        <div className="absolute bottom-6 right-4 z-[999] text-right pointer-events-none">
          <h1 className="text-2xl md:text-3xl font-bold text-[#C4A265]/80 tracking-wider" style={{ fontFamily: "'Noto Serif SC', serif" }}>三论宗祖师大图谱</h1>
          <p className="text-xs text-slate-500 mt-1">{patriarchs.length}位祖师 · 丝绸之路 · 中观空义</p>
        </div>
      )}
    </div>
  );
