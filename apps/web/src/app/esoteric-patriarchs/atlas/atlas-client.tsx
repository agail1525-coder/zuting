...
const CONFIG: AtlasConfig = {
  religionKey: "esoteric", title: "密宗祖师大图谱", subtitle: "海上丝路 · 金胎两部", themeColor: "#C4A265",
  backUrl: "/esoteric-patriarchs", detailUrlPrefix: "/esoteric-patriarchs", defaultCenter: [30, 110], defaultZoom: 4,
  filters: [{ key: "all", name: "全部", color: "#C4A265" }],
  legendItems: [{ name: "密宗", color: "#C4A265" }, { name: "真言宗", color: "#D4B87A" }],
  schoolColors: SCHOOL_COLORS, typeLabels: DEFAULT_TYPE_LABELS, loadingText: "正在加载密宗祖师大图谱...",
};
...
export default function AtlasClient({ patriarchs }: { patriarchs: Patriarch[] }) {
  ...
  return (
    <div className="fixed inset-0 bg-[#0f172a]">
      ...
      {!selectedId && <AtlasLegend title="密宗 · 真言宗" items={CONFIG.legendItems} themeColor={CONFIG.themeColor} />}
      ...
      {!selectedId && (
        <div className="absolute bottom-6 right-4 z-[999] text-right pointer-events-none">
          <h1 className="text-2xl md:text-3xl font-bold text-[#C4A265]/80 tracking-wider" style={{ fontFamily: "'Noto Serif SC', serif" }}>密宗祖师大图谱</h1>
          <p className="text-xs text-slate-500 mt-1">{patriarchs.length}位祖师 · 海上丝路 · 金胎两部</p>
        </div>
      )}
    </div>
  );
