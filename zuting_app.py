...
def _font(sz, bold=False):
    for n in (["msyhbd.ttc", "msyh.ttc"] if bold else ["msyh.ttc", "msyhbd.ttc"]):
        try:
...
def compose_site(site_name, religion, country, poem_orig, poem_src, poem_cn,
    ...
    draw.text((20, 14), header, fill=(255, 255, 255), font=f20)
    ...
    if has_geo:
        ...
        draw.text((45, 56), geo_line, fill=(160, 220, 255), font=f11)

    # 教义区
    cx = 55
    _draw_wrapped(draw, poem_orig, cx, 108, w - 60 - cx, f20, (255, 255, 255))
    draw.text((cx, 175), f"— {poem_src}", fill=style["color"], font=f15)
    _draw_wrapped(draw, poem_cn, cx, 200, w - 60 - cx, f17, (220, 220, 200))
    if chant_info:
        draw.text((cx, 270), f"♫ {chant_info['name']}: {chant_info['text'][:25]}...", fill=(180, 220, 180), font=f13)
    draw.text((cx, 290), f"{style['symbol']}  {religion}经典", fill=(180, 180, 180), font=f13)
...
def compose_patriarch(patriarch, teaching, religion, category, message, wisdom, count, chant_info, site_info=None):
    ...
    if has_geo:
        ...
        if site_info['weather']:
            geo_parts.append(site_info['weather'])
        draw.text((45, 56), "  |  ".join(geo_parts), fill=(160, 220, 255), font=f11)

    # 祖师信息区
    cx = 55
    draw.text((cx, 108), f"👤 {p_name}", fill=(255, 255, 255), font=f24)
    draw.text((cx, 143), f"{p_title}  ·  {p_era}", fill=style["color"], font=f15)
...
def compose_temple(temple, teaching, religion, category, message, wisdom, count, chant_info, travel, site_info=None):
    ...
    # 祖庭信息区
    cx = 55
    draw.text((cx, 108), f"🏛 {t_name}", fill=(255, 255, 255), font=f24)
...
def compose_seal(seal, health_key, health_val, mindful_text, wisdom, count):
    ...
    # 顶部: 系列名 + 印号
    draw.text((20, 12), f"🪷 曹溪愿命 · {series}", fill=(255, 255, 255), font=f20)
    draw.text((w - 180, 12), f"[印修] 第{count}次  {datetime.now().strftime('%H:%M')}", fill=(180, 180, 180), font=f13)

    # 印名 (大标题)
    seal_title = f"第{seal['id']}印 · {seal['name']}"
    draw.text((30, 60), seal_title, fill=sc, font=f28)

    # 偈语框 — 金色大字居中
    poem_lines = seal["poem"].split("\n")
    poem_y = 105
    # 偈语背景框
    draw.rectangle([(20, poem_y - 8), (w - 20, poem_y + len(poem_lines) * 38 + 8)],
                   fill=(30, 20, 45), outline=(cr, cg, cb, 100))
    for pl in poem_lines:
        pl = pl.strip()
        if not pl:
            continue
        bb = draw.textbbox((0, 0), pl, font=f24)
        tw = bb[2] - bb[0]
        draw.text(((w - tw) // 2, poem_y), pl, fill=(255, 215, 80), font=f24)
        poem_y += 38

    # 印义精要
    ey = poem_y + 15
    draw.text((30, ey), "📖 印义：", fill=sc, font=f17)
    ey = _draw_wrapped(draw, seal["essence"][:100], 30, ey + 25, w - 60, f15, (200, 200, 210))

    # 今日修持
    ey += 8
    draw.text((30, ey), "🕯 今日修持：", fill=(255, 200, 150), font=f17)
    ey = _draw_wrapped(draw, seal["practice"][:80], 30, ey + 25, w - 60, f13, (180, 180, 190))

    # 发愿词 (节选)
    if seal["vow"]:
        ey += 5
        vow_short = seal["vow"].replace("\n", " ")[:60] + "..."
        draw.text((30, ey), f"📜 {vow_short}", fill=(150, 170, 200), font=f13)
        ey += 22

    # 健康提醒区 (修行化)
    hy = max(ey + 10, h - 145)
    draw.line([(20, hy), (w - 20, hy)], fill=(60, 50, 80))
    hy += 8
    draw.text((30, hy), health_key, fill=sc, font=f20)
    draw.text((30, hy + 30), mindful_text, fill=(180, 190, 200), font=f15)

    # 小鸿修行指引 (底部)
    xh = compose_xiaohong_wisdom(seal)
    xiaohong_msg = xh.get("guidance", "")
    draw.text((20, h - 100), f"🐦 小鸿: {xiaohong_msg}", fill=(200, 180, 255), font=f13)

    # 底部按钮
    bt = "知道了，继续修行！ 🪷"
    bb = draw.textbbox((0, 0), bt, font=f20)
    draw.text(((w - (bb[2] - bb[0])) // 2, h - 48), bt, fill=sc, font=_font(18, True))
    draw.text((w - 50, h - 48), f"{POPUP_DURATION_SEC}s", fill=(150, 150, 150), font=f13)
