"""
全球宗教跨宗融合数据库 V8.0
12大信仰 × 200+圣地 × 120+祖庭 × 130+祖师 × 200+祖训 × 旅游攻略 × 音效
数据源: 道德经81章·金刚经·圣经·古兰经·薄伽梵歌·吠陀经·论语·古鲁格兰特萨希卜 等核心经典
"""

# ── 宗教配色 + 符号 ────────────────────────────────────
RELIGION_STYLES = {
    "佛教":     {"symbol": "☸", "color": (255, 195, 0),   "color2": (200, 150, 0)},
    "道教":     {"symbol": "☯", "color": (100, 200, 180), "color2": (60, 150, 130)},
    "基督教":   {"symbol": "✝", "color": (180, 50, 50),   "color2": (140, 30, 30)},
    "伊斯兰教": {"symbol": "☪", "color": (0, 140, 70),    "color2": (0, 100, 50)},
    "印度教":   {"symbol": "ॐ", "color": (230, 120, 20),  "color2": (190, 90, 10)},
    "犹太教":   {"symbol": "✡", "color": (50, 80, 180),   "color2": (30, 50, 140)},
    "儒家":     {"symbol": "儒", "color": (160, 50, 50),   "color2": (120, 30, 30)},
    "锡克教":   {"symbol": "☬", "color": (230, 150, 30),  "color2": (190, 120, 20)},
    "神道教":   {"symbol": "⛩", "color": (200, 50, 50),   "color2": (160, 30, 30)},
    "藏传佛教": {"symbol": "☸", "color": (150, 30, 30),   "color2": (120, 20, 20)},
    "原住民灵性":{"symbol": "◉", "color": (160, 100, 40),  "color2": (120, 70, 20)},
    "巴哈伊教": {"symbol": "✦", "color": (100, 60, 140),  "color2": (70, 40, 110)},
}

# ── 圣地数据 ──────────────────────────────────────────
# (圣地名, 宗教, 国家, 音效, 搜索词,
#  教义原文, 教义出处, 教义中文)

HOLY_SITES = [
    # ════ 佛教 (8) ════
    ("菩提伽耶", "佛教", "印度", "singing_bowl",
     "Bodh Gaya Mahabodhi Temple India",
     "诸恶莫作，众善奉行，自净其意",
     "《法句经》",
     "不做坏事，多行善事，净化心灵"),

    ("布达拉宫", "佛教", "中国", "singing_bowl",
     "布达拉宫 拉萨 风景",
     "色即是空，空即是色",
     "《心经》",
     "万物表象与本质相互依存"),

    ("吴哥窟", "佛教", "柬埔寨", "singing_bowl",
     "Angkor Wat Cambodia sunrise",
     "千里之行，始于足下之觉知",
     "巴利经典",
     "觉知当下的每一步"),

    ("法隆寺", "佛教", "日本", "temple_bell",
     "Horyuji Temple Japan Nara",
     "以和为贵，无忤为宗",
     "《十七条宪法》",
     "和谐为贵，不要对抗"),

    ("五台山", "佛教", "中国", "temple_bell",
     "五台山 寺庙 佛教",
     "应无所住而生其心",
     "《金刚经》",
     "不执着于任何事物，保持心灵自由"),

    ("仰光大金塔", "佛教", "缅甸", "singing_bowl",
     "Shwedagon Pagoda Myanmar golden",
     "恨不止恨，唯爱止恨",
     "《法句经》",
     "仇恨无法终止仇恨，唯有爱能化解"),

    ("婆罗浮屠", "佛教", "印尼", "singing_bowl",
     "Borobudur Temple Indonesia sunrise",
     "一花一世界，一叶一菩提",
     "《华严经》",
     "微小事物中蕴含整个宇宙"),

    ("敦煌莫高窟", "佛教", "中国", "guqin_pluck",
     "敦煌莫高窟 飞天壁画",
     "过去心不可得，现在心不可得，未来心不可得",
     "《金刚经》",
     "活在当下，不执过去未来"),

    # ════ 道教 (6) ════
    ("武当山", "道教", "中国", "bamboo_flute",
     "武当山 道观 风景",
     "道可道，非常道；名可名，非常名",
     "《道德经》第一章",
     "真正的道理超越语言的表达"),

    ("青城山", "道教", "中国", "bamboo_flute",
     "青城山 道观 幽静",
     "上善若水，水善利万物而不争",
     "《道德经》第八章",
     "最高的善如水一般，滋养万物不争名利"),

    ("龙虎山", "道教", "中国", "guqin_pluck",
     "龙虎山 天师府 道教",
     "人法地，地法天，天法道，道法自然",
     "《道德经》第二十五章",
     "人效法大地，最终归于自然"),

    ("茅山", "道教", "中国", "bamboo_flute",
     "茅山 道观 江苏",
     "致虚极，守静笃",
     "《道德经》第十六章",
     "让心灵虚空到极点，守住宁静"),

    ("崂山", "道教", "中国", "wind_chimes",
     "青岛崂山 海上仙山",
     "天地与我并生，万物与我为一",
     "《庄子·齐物论》",
     "天地万物与我本是一体"),

    ("白云观", "道教", "中国", "guqin_pluck",
     "北京白云观 道教",
     "知足不辱，知止不殆，可以长久",
     "《道德经》第四十四章",
     "知道满足就不会受辱，知道适可而止才能长久"),

    # ════ 基督教 (8) ════
    ("梵蒂冈圣彼得大教堂", "基督教", "意大利", "pipe_organ",
     "Vatican St Peters Basilica Rome",
     "God is love, and whoever abides in love abides in God",
     "1 John 4:16",
     "神就是爱，住在爱里面的就是住在神里面"),

    ("圣家堂", "基督教", "西班牙", "choir_hymn",
     "Sagrada Familia Barcelona Gaudi",
     "Faith is the substance of things hoped for",
     "Hebrews 11:1",
     "信就是所望之事的实底，未见之事的确据"),

    ("巴黎圣母院", "基督教", "法国", "pipe_organ",
     "Notre Dame Paris Cathedral",
     "Love is patient, love is kind",
     "1 Corinthians 13:4",
     "爱是恒久忍耐，又有恩慈"),

    ("圣索菲亚大教堂", "基督教", "土耳其", "choir_hymn",
     "Hagia Sophia Istanbul interior",
     "The truth shall set you free",
     "John 8:32",
     "真理必叫你们得以自由"),

    ("科隆大教堂", "基督教", "德国", "pipe_organ",
     "Cologne Cathedral Germany gothic",
     "Be still, and know that I am God",
     "Psalm 46:10",
     "你们要休息，要知道我是神"),

    ("威斯敏斯特教堂", "基督教", "英国", "choir_hymn",
     "Westminster Abbey London interior",
     "For everything there is a season",
     "Ecclesiastes 3:1",
     "凡事都有定期，天下万务都有定时"),

    ("耶路撒冷圣墓教堂", "基督教", "以色列", "choir_hymn",
     "Church Holy Sepulchre Jerusalem",
     "Peace I leave with you; my peace I give to you",
     "John 14:27",
     "我留下平安给你们，我将我的平安赐给你们"),

    ("米兰大教堂", "基督教", "意大利", "pipe_organ",
     "Milan Cathedral Duomo Italy",
     "Do to others as you would have them do to you",
     "Luke 6:31",
     "你们愿意人怎样待你们，你们也要怎样待人"),

    # ════ 伊斯兰教 (6) ════
    ("麦加禁寺", "伊斯兰教", "沙特", "adhan_call",
     "Masjid al-Haram Mecca Kaaba night",
     "Bismillah ir-Rahman ir-Rahim",
     "《古兰经》开端章",
     "奉至仁至慈的真主之名"),

    ("麦地那先知寺", "伊斯兰教", "沙特", "adhan_call",
     "Prophet Mosque Medina green dome",
     "إن مع العسر يسرا",
     "《古兰经》94:6",
     "困难之后必有容易"),

    ("阿尔罕布拉宫", "伊斯兰教", "西班牙", "adhan_call",
     "Alhambra Palace Granada Spain",
     "Lā ghāliba illā Allāh",
     "阿尔罕布拉宫铭文",
     "唯有真主是胜利者"),

    ("蓝色清真寺", "伊斯兰教", "土耳其", "adhan_call",
     "Blue Mosque Sultan Ahmed Istanbul",
     "خير الناس أنفعهم للناس",
     "圣训",
     "最好的人是对人们最有益的人"),

    ("圆顶清真寺", "伊斯兰教", "以色列", "adhan_call",
     "Dome of the Rock Jerusalem golden",
     "وَتَعَاوَنُوا عَلَى الْبِرِّ وَالتَّقْوَىٰ",
     "《古兰经》5:2",
     "你们当在善事和敬畏上互相合作"),

    ("泰姬陵", "伊斯兰教", "印度", "adhan_call",
     "Taj Mahal India sunrise reflection",
     "و من آياته أن خلق لكم من أنفسكم أزواجا",
     "《古兰经》30:21",
     "他的迹象之一是为你们从你们自身中创造了配偶"),

    # ════ 印度教 (6) ════
    ("瓦拉纳西恒河", "印度教", "印度", "sitar_drone",
     "Varanasi Ganges River ceremony India",
     "योगस्थ: कुरु कर्माणि",
     "《薄伽梵歌》2:48",
     "安住于瑜伽中行动——做事不执着于结果"),

    ("巴厘岛母庙", "印度教", "印尼", "sitar_drone",
     "Besakih Temple Bali Indonesia",
     "Tat tvam asi",
     "《歌者奥义书》",
     "你就是那个——万物本质相同"),

    ("科纳克太阳神庙", "印度教", "印度", "sitar_drone",
     "Konark Sun Temple India Odisha",
     "Aham Brahmāsmi",
     "《大森林奥义书》",
     "我即是梵——个体灵魂与宇宙本源合一"),

    ("尼泊尔帕斯帕提纳", "印度教", "尼泊尔", "sitar_drone",
     "Pashupatinath Temple Nepal Kathmandu",
     "Vasudhaiva Kutumbakam",
     "《大奥义书》",
     "世界是一个大家庭"),

    ("吴哥窟印度教", "印度教", "柬埔寨", "sitar_drone",
     "Angkor Wat Cambodia Hindu temple",
     "Satyam eva jayate",
     "《蒙查羯奥义书》",
     "唯有真理必胜"),

    ("阿克萨达姆神庙", "印度教", "印度", "sitar_drone",
     "Akshardham Temple Delhi India",
     "Lokah Samastah Sukhino Bhavantu",
     "印度教祈祷文",
     "愿一切众生幸福安康"),

    # ════ 犹太教 (4) ════
    ("耶路撒冷哭墙", "犹太教", "以色列", "shofar_horn",
     "Western Wall Jerusalem prayer",
     "Shema Yisrael Adonai Eloheinu Adonai Echad",
     "《申命记》6:4",
     "以色列啊，你要听！耶和华我们的神是独一的主"),

    ("马萨达要塞", "犹太教", "以色列", "shofar_horn",
     "Masada Fortress Israel desert sunrise",
     "Tzedek tzedek tirdof",
     "《申命记》16:20",
     "公义，公义，你要追求"),

    ("死海古卷洞", "犹太教", "以色列", "shofar_horn",
     "Dead Sea Scrolls Qumran caves Israel",
     "V'ahavta l'reacha kamocha",
     "《利未记》19:18",
     "爱人如己"),

    ("锡安山", "犹太教", "以色列", "shofar_horn",
     "Mount Zion Jerusalem Israel",
     "Tikkun Olam",
     "犹太传统",
     "修复世界——每个人都有责任让世界变得更好"),

    # ════ 儒家 (4) ════
    ("曲阜孔庙", "儒家", "中国", "temple_bell",
     "曲阜孔庙 山东",
     "己所不欲，勿施于人",
     "《论语·卫灵公》",
     "自己不想要的，不要强加给别人"),

    ("岳麓书院", "儒家", "中国", "guqin_pluck",
     "岳麓书院 长沙 湖南",
     "学而不思则罔，思而不学则殆",
     "《论语·为政》",
     "只学不思就迷惘，只思不学就危险"),

    ("白鹿洞书院", "儒家", "中国", "guqin_pluck",
     "白鹿洞书院 庐山 江西",
     "三人行，必有我师焉",
     "《论语·述而》",
     "三人同行，其中必有我可以学习的人"),

    ("北京国子监", "儒家", "中国", "temple_bell",
     "北京国子监 孔庙 辟雍",
     "修身齐家治国平天下",
     "《大学》",
     "先修养自身，再齐家治国平天下"),

    # ════ 锡克教 (3) ════
    ("阿姆利则金庙", "锡克教", "印度", "choir_hymn",
     "Golden Temple Amritsar India night",
     "Ik Onkar — There is One God",
     "《古鲁·格兰特·萨希卜》",
     "万物同源——只有一个创造者"),

    ("旁遮普古鲁圣地", "锡克教", "印度", "choir_hymn",
     "Gurdwara Bangla Sahib Delhi Sikh",
     "Kirat Karo, Naam Japo, Vand Chhako",
     "锡克教三大支柱",
     "诚实劳作，冥想神名，与人分享"),

    ("锡克教圣殿", "锡克教", "印度", "choir_hymn",
     "Harmandir Sahib Amritsar golden reflection",
     "Recognize the whole human race as one",
     "古鲁·戈宾德·辛格",
     "视全人类为一体"),

    # ════ 神道教 (4) ════
    ("伊势神宫", "神道教", "日本", "taiko_drum",
     "Ise Grand Shrine Japan torii",
     "神は人の敬により威を増す",
     "神道教训",
     "神因人的敬意而增辉——敬畏之心使世界光明"),

    ("严岛神社", "神道教", "日本", "wind_chimes",
     "Itsukushima Shrine floating torii gate",
     "清き明き直き心",
     "神道四德",
     "清净、光明、正直之心"),

    ("伏见稻荷大社", "神道教", "日本", "taiko_drum",
     "Fushimi Inari Shrine thousand torii gates",
     "森羅万象に神が宿る",
     "神道万物有灵",
     "万物之中皆有神灵——敬畏每一个存在"),

    ("明治神宫", "神道教", "日本", "wind_chimes",
     "Meiji Shrine Tokyo forest path",
     "和を以て貴しとなす",
     "《日本书纪》",
     "以和为贵——和谐是最珍贵的价值"),

    # ════ 藏传佛教 (4) ════
    ("拉萨大昭寺", "藏传佛教", "中国", "singing_bowl",
     "大昭寺 拉萨 藏传佛教",
     "嗡嘛呢叭咪吽",
     "六字真言",
     "莲花中的珍宝——慈悲与智慧合一"),

    ("甘丹寺", "藏传佛教", "中国", "singing_bowl",
     "甘丹寺 西藏 藏传佛教",
     "若欲知佛性，当观时节因缘",
     "宗喀巴大师",
     "要了解真理，要观察因缘时节"),

    ("色达喇荣五明佛学院", "藏传佛教", "中国", "singing_bowl",
     "色达喇荣五明佛学院 红房子",
     "自利利他，悲智双运",
     "藏传佛教教义",
     "利益自己也利益他人，悲悯与智慧并行"),

    ("不丹虎穴寺", "藏传佛教", "不丹", "singing_bowl",
     "Tiger Nest Monastery Bhutan Paro cliff",
     "幸福不在于拥有多少，而在于放下多少",
     "不丹智慧",
     "幸福源于放下执着"),

    # ════ 原住民灵性 (4) ════
    ("乌鲁鲁巨石", "原住民灵性", "澳大利亚", "didgeridoo",
     "Uluru Ayers Rock Australia sunset",
     "We are all visitors to this time, this place",
     "澳大利亚原住民谚语",
     "我们都是这个时空的过客——珍惜当下"),

    ("马丘比丘", "原住民灵性", "秘鲁", "didgeridoo",
     "Machu Picchu Peru Inca ruins mountains",
     "Ama sua, ama llulla, ama quella",
     "印加帝国三戒律",
     "不偷窃，不说谎，不懒惰"),

    ("巨石阵", "原住民灵性", "英国", "ocean_waves",
     "Stonehenge England sunset ancient",
     "The earth does not belong to us; we belong to the earth",
     "西雅图酋长",
     "大地不属于我们，我们属于大地"),

    ("奇琴伊察", "原住民灵性", "墨西哥", "didgeridoo",
     "Chichen Itza pyramid Mexico Maya",
     "In Lak'ech Ala K'in",
     "玛雅问候语",
     "我是另一个你——你我本是一体"),

    # ════ 巴哈伊教 (3) ════
    ("海法空中花园", "巴哈伊教", "以色列", "choir_hymn",
     "Bahai Gardens Haifa Israel terraces",
     "The earth is but one country, and mankind its citizens",
     "巴哈欧拉",
     "大地只是一个国家，人类都是其公民"),

    ("新德里莲花寺", "巴哈伊教", "印度", "choir_hymn",
     "Lotus Temple New Delhi Bahai India",
     "Let your vision be world-embracing",
     "巴哈欧拉",
     "让你的目光拥抱整个世界"),

    ("威尔梅特灵曦堂", "巴哈伊教", "美国", "choir_hymn",
     "Wilmette Bahai Temple Chicago dome",
     "So powerful is the light of unity that it can illuminate the whole earth",
     "巴哈欧拉",
     "团结之光如此强大，足以照亮整个大地"),
]

# ── 健康提醒（融入宗教智慧语境）────────────────────────
HEALTH_REMINDERS = {
    # ── 颈椎保健 ──
    "颈椎放松": [
        "头缓慢向右倾斜，右手轻压左侧头部，保持15秒换边——办公族颈椎救星",
        "下巴收紧贴向胸口，双手十指交叉放脑后轻压，拉伸颈后肌群15秒",
        "头部画∞字：先向左画圈再向右画圈，各5次，速度要慢像打太极",
        "耸肩运动：双肩用力耸到最高，保持3秒猛然放下，重复10次释放紧张",
        "仰头望天花板5秒→低头看肚脐5秒→左转看肩膀5秒→右转5秒，循环3轮",
        "颈椎米字操：用下巴在空中写'米'字，每笔到位，做完整个脖子都松了",
        "双手握拳放下巴下方，下巴用力往下压拳头，拳头顶住不动，对抗10秒×5组",
        "转头看身后：身体不动，头慢慢转向右后方看墙角，保持10秒换边",
    ],
    # ── 肩背运动 ──
    "肩背舒展": [
        "扩胸运动：双臂向后展开如雄鹰展翅，肩胛骨夹紧保持5秒，做10次",
        "猫牛式：坐在椅子上，吸气挺胸抬头(牛式)→呼气弓背低头(猫式)，做8次",
        "肩膀画圈：双肩同时向前画大圈10次→向后画大圈10次，幅度越大越好",
        "背后握手：右手从上方、左手从下方在背后尝试握手，保持15秒换边",
        "门框拉伸：双手撑住门框两侧，身体前倾，拉伸胸肌和前肩，保持20秒",
        "蝴蝶展翅：双手放耳后，手肘尽量往后打开→再合拢碰到一起，做15次",
        "耸肩画圈：肩膀耸起→向后→放下→向前，画完整的圆，正反各10圈",
        "天使翅膀：背靠墙站立，双臂贴墙做投降姿势，上下滑动像雪地天使10次",
    ],
    # ── 腰部运动 ──
    "腰部活力": [
        "坐姿转体：双手抱头，上身向右旋转到极限保持5秒→向左，各做8次",
        "骨盆前后倾：坐椅子上，骨盆前倾挺腰→后倾弓背，有节奏地做20次",
        "坐姿侧弯：右手举过头顶向左侧弯，拉伸右侧腰，保持15秒换边",
        "站立体前屈：双脚与肩同宽，缓慢弯腰手碰脚尖(碰不到没关系)，保持20秒",
        "腰部画圈：双手叉腰，骨盆画大圈，顺时针10圈→逆时针10圈",
        "椅子扭腰：坐稳后双手抓椅背右侧，身体尽量向右转看后面，保持15秒换边",
        "站姿挺腰：双手撑腰后方，缓慢向后仰，拉伸腹部和腰前肌群，保持10秒×3",
        "婴儿式放松：跪坐在地上(或趴在桌上)，额头贴地/桌面，双手前伸放松30秒",
    ],
    # ── 手腕手指 ──
    "手腕护理": [
        "腕部画圈：双手握拳，手腕顺时针画圈20次→逆时针20次，防鼠标手",
        "手指钢琴操：十指在桌面上快速弹击如弹钢琴，从小指到拇指来回10轮",
        "祈祷拉伸：双手合十放胸前，手肘慢慢抬高让手腕弯到90°，保持15秒",
        "反向祈祷：双手手背相对放胸前，向下压拉伸手腕外侧，保持15秒",
        "握力训练：用力握拳5秒→猛然张开手指伸到最大5秒，重复15次",
        "拇指对指：拇指依次轻触食指→中指→无名指→小指→再倒回来，做10轮",
        "手臂翻转：手臂前伸，掌心朝上→翻转朝下→再翻回来，连续做20次",
        "橡皮筋训练：五指撑开套着的橡皮筋，张合10次——预防腱鞘炎利器",
    ],
    # ── 腿部运动 ──
    "腿部激活": [
        "坐姿抬腿：坐在椅子上，伸直右腿平举保持10秒→放下换左腿，各做8次",
        "脚踝画圈：抬起一只脚，脚踝顺时针画圈15次→逆时针15次，换脚",
        "原地高抬腿：站起来原地高抬腿跑30秒，心率马上上来，醒神利器",
        "椅子深蹲：站到椅子前，臀部慢慢下坐直到碰到椅面→站起来，做15次",
        "小腿拉伸：面对墙壁弓步站立，后脚跟踩实拉伸小腿，每侧保持20秒",
        "坐姿踮脚：脚尖踮起保持3秒→脚跟踮起保持3秒→交替做20次",
        "靠墙静蹲：背靠墙滑下至大腿平行地面，坚持30秒——腿抖了就对了",
        "单腿站立：闭眼单脚站立30秒(扶墙也行)，锻炼平衡力和核心",
    ],
    # ── 全身运动 ──
    "全身唤醒": [
        "八段锦第一式'两手托天理三焦'：双手交叉上托过头顶，脚跟踮起，做8次",
        "开合跳30秒：双脚跳开双手举过头→跳回双手放下，简单有效全身热身",
        "波比跳5个：站立→蹲下撑地→跳脚向后→跳脚收回→站起跳起——全身燃脂王",
        "俯卧撑10个：标准版或跪姿版都行，激活胸肌手臂核心",
        "平板支撑30秒：手肘撑地全身绷直像一块木板，核心力量之王",
        "太极云手：双脚与肩同宽微蹲，双手在身前缓慢画圆，重心左右移动，做2分钟",
        "拉伸下犬式：双手双脚撑地臀部抬高，身体呈倒V字形，保持30秒拉伸全身",
        "弹力带划船：把弹力带挂门把手上，坐着做划船动作，练背防驼背",
    ],
    # ── 眼部保健 ──
    "眼部保健": [
        "20-20-20法则：每20分钟看20英尺(6米)外的东西20秒，缓解眼疲劳",
        "眼球运动：眼睛向上看→右看→下看→左看→画完整圆圈，正反各5圈",
        "远近交替：看远处10秒→看手指(30cm)10秒→交替做10次，锻炼睫状肌",
        "手掌温敷：双手搓热后轻轻覆盖闭合的眼睛，感受温暖，保持30秒",
        "眨眼操：快速眨眼20次→紧闭双眼5秒→睁开，重复3组——干眼症克星",
        "眼保健操：按揉太阳穴→刮上眼眶→按压四白穴→揉天应穴，经典永不过时",
        "看绿色植物1分钟：把目光投向窗外的树木或桌上的绿植，放松视神经",
        "闭目养神：闭上眼睛，想象自己站在广阔的草原上眺望远方，保持1分钟",
    ],
    # ── 办公桌微健身 ──
    "桌边健身": [
        "桌面俯卧撑：双手撑桌边与肩同宽，做倾斜俯卧撑15个，简单但有效",
        "椅子臂屈伸：双手撑椅子边缘，臀部悬空，手臂弯曲下沉→撑起，做10个",
        "坐姿提踵：坐着双脚平放地面，脚后跟用力抬起→放下，连续做30次",
        "坐姿夹纸：双膝之间夹一张A4纸(或书本)，大腿内侧用力夹紧15秒×5组",
        "桌下踢腿：坐着把腿在桌下交替踢出伸直，像骑自行车一样，做30秒",
        "坐姿收腹：坐直，吸气收紧腹部保持10秒→呼气放松，做15次练核心",
        "手臂弯举：拿起水瓶当哑铃，做二头肌弯举15次×3组，左右手都练",
        "等距推墙：双手推墙用力10秒→放松5秒，重复8次，激活胸肌和手臂",
    ],
    # ── 放松与冥想 ──
    "正念放松": [
        "身体扫描：从脚趾开始往上觉察每个部位有没有紧张，一一放松它",
        "渐进式放松：握紧右拳5秒→松开感受放松→左拳→双脚→全身，逐个部位",
        "5-4-3-2-1着陆法：看5样东西→摸4样→听3种声音→闻2种气味→尝1种味道",
        "正念饮水：端起水杯，感受杯子的温度、水的重量，慢慢喝一口专注品味",
        "3分钟冥想：闭眼坐正，只关注呼吸，杂念来了不理它让它飘走",
        "感恩练习：闭上眼想3件今天值得感恩的事，哪怕很小——重塑正向神经回路",
        "肩颈自我按摩：右手搓揉左肩斜方肌30秒→换边，遇到硬结多按几下",
        "百会穴点按：头顶正中百会穴用中指按压旋转30秒，提神醒脑通气血",
    ],
    # ── 喝水提醒 ──
    "喝水时间": [
        "身体是灵魂的神殿，请以清水滋养它——现在就喝一杯",
        "你已经多久没喝水了？起身倒杯温水，边走边喝一举两得",
        "中医说'晨起一杯水胜过十副药'——不管几点先喝一杯再说",
        "脱水会让专注力下降12%——喝杯水，大脑会感谢你的",
        "泡杯绿茶或花茶，茶香本身就是一种疗愈",
        "柠檬水、蜂蜜水、淡盐水——给白水加点变化今天换个口味吧",
    ],
}

# ── 通用激励语（跨宗教融合）──────────────────────────
UNIVERSAL_WISDOM = [
    "所有信仰都教导我们：善待自己的身体",
    "健康是所有修行的基石",
    "身体是灵魂在这个世界的居所",
    "动静之间，找到内心的平衡",
    "每一次呼吸都是宇宙的礼物",
    "爱自己，才能更好地爱世界",
    "身体健康，心灵才能自由翱翔",
    "所有宗教的共同语言：爱与慈悲",
    "照顾好身体，才能走更远的路",
    "万法归一：健康、平和、慈悲",
    "世界很大，先照顾好自己",
    "休息不是懒惰，是对生命的尊重",
]


# ══════════════════════════════════════════════════════
#  V5.0 新增: 祖庭 (Ancestral Temples / Founding Sites)
# ══════════════════════════════════════════════════════
# (祖庭名, 宗教, 国家, 创建年代, 历史意义, 搜索词)

ANCESTRAL_TEMPLES = [
    # ════ 佛教 ════
    ("鹿野苑", "佛教", "印度", "公元前528年",
     "释迦牟尼初转法轮之地，佛教第一次说法处",
     "Sarnath Deer Park India Buddhist temple"),
    ("灵鹫山", "佛教", "印度", "公元前6世纪",
     "佛陀宣说法华经、般若经之圣地",
     "Vulture Peak Rajgir India Buddhism"),
    ("祇园精舍", "佛教", "印度", "公元前6世纪",
     "佛陀驻锡最久的道场，众多经典在此宣说",
     "Jetavana Monastery Sravasti India ruins"),

    # ════ 道教 ════
    ("鹤鸣山", "道教", "中国", "东汉142年",
     "张道陵创立天师道之地，道教第一座祖庭",
     "鹤鸣山 道教祖庭 成都"),
    ("终南山", "道教", "中国", "远古",
     "老子著《道德经》之地，道教修行圣地",
     "终南山 道教 修行 西安"),
    ("楼观台", "道教", "中国", "周朝",
     "老子讲经著书之地，道教最早宫观",
     "楼观台 道教 老子 讲经"),

    # ════ 基督教 ════
    ("伯利恒主诞堂", "基督教", "巴勒斯坦", "公元326年",
     "耶稣基督降生之地，基督教最古老教堂之一",
     "Church of Nativity Bethlehem Palestine"),
    ("各各他", "基督教", "以色列", "公元1世纪",
     "耶稣受难十字架之地，基督教信仰核心圣地",
     "Golgotha Calvary Jerusalem holy site"),
    ("安提阿", "基督教", "土耳其", "公元1世纪",
     "门徒首次被称为基督徒的城市，早期教会中心",
     "Antakya Antioch Church cave Turkey"),

    # ════ 伊斯兰教 ════
    ("希拉山洞", "伊斯兰教", "沙特", "公元610年",
     "先知穆罕默德首次接受启示之地",
     "Cave Hira Mecca Saudi Arabia mountain"),
    ("库巴清真寺", "伊斯兰教", "沙特", "公元622年",
     "伊斯兰教第一座清真寺",
     "Quba Mosque Medina Saudi Arabia first mosque"),

    # ════ 印度教 ════
    ("哈里德瓦尔", "印度教", "印度", "远古",
     "恒河出山入平原之地，印度教七大圣城之首",
     "Haridwar India Ganges Hindu pilgrimage"),
    ("瑞诗凯诗", "印度教", "印度", "远古",
     "瑜伽之都，古代圣人苦修之地",
     "Rishikesh India yoga capital temple"),

    # ════ 犹太教 ════
    ("圣殿山", "犹太教", "以色列", "公元前957年",
     "所罗门圣殿所在地，犹太教最神圣之地",
     "Temple Mount Jerusalem Israel holy site"),
    ("希伯伦先祖墓", "犹太教", "以色列", "公元前2世纪",
     "亚伯拉罕、以撒、雅各安葬之地",
     "Cave of Patriarchs Hebron Israel"),

    # ════ 儒家 ════
    ("曲阜阙里", "儒家", "中国", "春秋",
     "孔子故里，儒学发源地",
     "曲阜 孔子故里 阙里 山东"),
    ("洙泗书堂", "儒家", "中国", "春秋",
     "孔子讲学之地，杏坛教化天下",
     "曲阜 杏坛 孔子讲学"),

    # ════ 锡克教 ════
    ("卡塔普尔", "锡克教", "巴基斯坦", "1522年",
     "古鲁那纳克晚年居住讲道之地",
     "Kartarpur Sahib Pakistan Sikh temple"),
    ("南德德", "锡克教", "印度", "1708年",
     "第十代古鲁戈宾德·辛格圆寂之地",
     "Hazur Sahib Nanded India Sikh shrine"),

    # ════ 神道教 ════
    ("出云大社", "神道教", "日本", "远古",
     "日本最古老神社之一，大国主神之居所",
     "Izumo Taisha Shrine Japan Shimane"),
    ("高天原", "神道教", "日本", "神话",
     "天照大神居住的天上世界，神道教创世之源",
     "Takamagahara Japan Shinto mythology shrine"),

    # ════ 藏传佛教 ════
    ("桑耶寺", "藏传佛教", "中国", "公元779年",
     "西藏第一座佛法僧三宝俱全的寺院",
     "桑耶寺 西藏 山南 藏传佛教 第一寺"),
    ("甘丹寺_祖庭", "藏传佛教", "中国", "1409年",
     "宗喀巴创建格鲁派之地，黄教祖庭",
     "甘丹寺 宗喀巴 格鲁派 祖庭"),

    # ════ 原住民灵性 ════
    ("梦时代圣地", "原住民灵性", "澳大利亚", "远古",
     "澳洲原住民梦时代创世的精神中心",
     "Aboriginal Dreamtime sacred site Australia"),
    ("太阳神庙库斯科", "原住民灵性", "秘鲁", "1438年",
     "印加帝国太阳神殿，黄金之城的精神中心",
     "Coricancha Temple Cusco Peru Inca sun temple"),

    # ════ 巴哈伊教 ════
    ("巴格达Ridvan花园", "巴哈伊教", "伊拉克", "1863年",
     "巴哈欧拉宣布使命之地，巴哈伊教创立圣地",
     "Ridvan Garden Baghdad Iraq Bahai holy"),
    ("阿卡监狱城", "巴哈伊教", "以色列", "1868年",
     "巴哈欧拉被囚禁并写下多部圣典之地",
     "Akka Acre Israel Bahai prison city"),
]


# ══════════════════════════════════════════════════════
#  V5.0 新增: 祖师 (Patriarchs / Founders)
# ══════════════════════════════════════════════════════
# (祖师名, 宗教, 时代, 尊号, 核心贡献, 名言, 搜索词)

PATRIARCHS = [
    # ════ 佛教 ════
    ("释迦牟尼", "佛教", "公元前563-483年", "世尊·佛陀",
     "创立佛教，悟道成佛，说法四十五年度化众生",
     "诸恶莫作，众善奉行，自净其意，是诸佛教",
     "Siddhartha Gautama Buddha statue"),
    ("龙树菩萨", "佛教", "公元150-250年", "第二佛陀",
     "创立中观学派，著《中论》阐明空性智慧",
     "以有空义故，一切法得成",
     "Nagarjuna Bodhisattva Buddhist master"),
    ("六祖慧能", "佛教", "638-713年", "禅宗六祖",
     "创顿悟法门，著《六祖坛经》，禅宗发扬光大",
     "菩提本无树，明镜亦非台，本来无一物，何处惹尘埃",
     "Huineng Sixth Patriarch Zen Buddhism"),

    # ════ 道教 ════
    ("老子", "道教", "春秋", "太上老君",
     "著《道德经》五千言，道家哲学创始人",
     "道可道，非常道；名可名，非常名",
     "Laozi Lao Tzu Tao Te Ching statue"),
    ("张道陵", "道教", "34-156年", "张天师·正一真人",
     "创立天师道(正一道)，道教第一代天师",
     "正一盟威之道，以清静为宗",
     "Zhang Daoling Celestial Master Taoism"),
    ("王重阳", "道教", "1112-1170年", "重阳真人",
     "创立全真道，倡三教合一，道教革新",
     "心中端正，无须装饰；行事光明，何惧黑暗",
     "Wang Chongyang Quanzhen Taoism founder"),

    # ════ 基督教 ════
    ("耶稣基督", "基督教", "公元前4-公元30年", "救世主·神子",
     "基督教创立者，传扬天国福音，十字架救赎",
     "我就是道路、真理、生命",
     "Jesus Christ painting Renaissance"),
    ("使徒保罗", "基督教", "5-67年", "外邦人使徒",
     "将基督教从犹太扩展到整个罗马帝国",
     "如今常存的有信、有望、有爱，其中最大的是爱",
     "Apostle Paul painting icon Christian"),
    ("奥古斯丁", "基督教", "354-430年", "恩典博士",
     "基督教神学奠基人，著《忏悔录》《上帝之城》",
     "你为自己创造了我们，我们的心不安息，直到安息在你里面",
     "Saint Augustine Hippo painting Christian"),

    # ════ 伊斯兰教 ════
    ("先知穆罕默德", "伊斯兰教", "570-632年", "封印先知·安拉的使者",
     "伊斯兰教创立者，接受《古兰经》启示",
     "求知，从摇篮到坟墓",
     "Prophet Muhammad calligraphy Islamic art"),
    ("阿布·伯克尔", "伊斯兰教", "573-634年", "第一任哈里发",
     "先知最亲密伙伴，首任正统哈里发",
     "善良不会因为谦虚而减少",
     "Abu Bakr Siddiq Islamic calligraphy"),

    # ════ 印度教 ════
    ("商羯罗", "印度教", "788-820年", "阿阇梨·世界导师",
     "创立不二论吠檀多，统一印度教各派",
     "梵我一如——个体灵魂与宇宙至上实在本质相同",
     "Adi Shankara Hindu philosopher statue India"),
    ("罗摩努阇", "印度教", "1017-1137年", "阿阇梨",
     "创限定不二论，强调对神的虔爱(Bhakti)",
     "虔诚的爱是通向解脱的最高道路",
     "Ramanuja Hindu philosopher statue"),

    # ════ 犹太教 ════
    ("亚伯拉罕", "犹太教", "约公元前2000年", "信心之父",
     "犹太教、基督教、伊斯兰教共同的先祖",
     "你要离开本地、本族、父家，往我所要指示你的地去",
     "Abraham patriarch biblical painting"),
    ("摩西", "犹太教", "约公元前1400年", "先知中最伟大者",
     "带领以色列人出埃及，在西奈山领受十诫",
     "以色列啊，你要听！耶和华我们的神是独一的主",
     "Moses Ten Commandments painting Sinai"),

    # ════ 儒家 ════
    ("孔子", "儒家", "前551-前479年", "至圣先师·万世师表",
     "创立儒学，有教无类，编纂六经",
     "己所不欲，勿施于人",
     "Confucius portrait statue Qufu"),
    ("孟子", "儒家", "前372-前289年", "亚圣",
     "发展仁政学说，提出性善论",
     "天将降大任于斯人也，必先苦其心志",
     "Mencius portrait Chinese philosopher"),
    ("朱熹", "儒家", "1130-1200年", "紫阳先生·朱子",
     "集理学大成，注释四书成为科举标准",
     "问渠那得清如许，为有源头活水来",
     "Zhu Xi Neo-Confucian philosopher portrait"),

    # ════ 锡克教 ════
    ("古鲁那纳克", "锡克教", "1469-1539年", "第一代古鲁",
     "锡克教创始人，倡一神论、平等、无私服务",
     "Ik Onkar——真理之名，创造者，无所畏惧，无有仇恨",
     "Guru Nanak Sikh founder painting"),
    ("古鲁阿尔詹", "锡克教", "1563-1606年", "第五代古鲁",
     "编纂锡克教圣典《古鲁·格兰特·萨希卜》",
     "甘露之名充满我的心",
     "Guru Arjan Sikh fifth guru painting"),

    # ════ 神道教 ════
    ("天照大神", "神道教", "神话", "太阳女神·皇祖神",
     "神道教最高神，日本皇室祖神，太阳化身",
     "以和为贵，万物共生",
     "Amaterasu Sun Goddess Japanese painting"),

    # ════ 藏传佛教 ════
    ("莲花生大士", "藏传佛教", "8世纪", "咕噜仁波切",
     "将密宗佛法传入西藏，降伏本土苯教神灵",
     "嗡阿吽 班杂 咕噜 贝玛 悉地吽",
     "Padmasambhava Guru Rinpoche thangka painting"),
    ("宗喀巴大师", "藏传佛教", "1357-1419年", "第二佛陀",
     "创立格鲁派(黄教)，著《菩提道次第广论》",
     "暇满人身极难得，既得能办人生利",
     "Tsongkhapa Yellow Hat Buddhism thangka"),
    ("阿底峡尊者", "藏传佛教", "982-1054年", "吉祥燃灯智",
     "著《菩提道灯论》，复兴西藏佛教",
     "诸法如梦如幻，如水中月影",
     "Atisha Buddhist master India Tibet painting"),

    # ════ 原住民灵性 ════
    ("梦时代祖灵", "原住民灵性", "远古", "创世祖灵",
     "澳洲原住民相信祖灵在梦时代创造了大地万物",
     "我们不拥有土地，土地拥有我们",
     "Aboriginal Dreamtime art painting Australia"),
    ("印加太阳神因蒂", "原住民灵性", "15世纪", "太阳神·印加守护者",
     "印加帝国至高神，太阳的化身",
     "Ama sua, ama llulla, ama quella——不偷窃，不说谎，不懒惰",
     "Inti Inca Sun God golden artwork Peru"),

    # ════ 巴哈伊教 ════
    ("巴哈欧拉", "巴哈伊教", "1817-1892年", "上帝的荣耀",
     "巴哈伊信仰创始人，宣告人类一体原则",
     "大地只是一个国家，人类都是其公民",
     "Bahaullah Bahai faith founder portrait"),
    ("巴孛", "巴哈伊教", "1819-1850年", "大门",
     "巴比教创始人，巴哈欧拉的先驱",
     "当黎明来临，你们要准备好迎接新的一天",
     "The Bab Bahai faith forerunner shrine"),
]


# ══════════════════════════════════════════════════════
#  V5.0 新增: 祖训 (Ancestral Teachings / Core Precepts)
# ══════════════════════════════════════════════════════
# (祖训名, 宗教, 出自祖师, 训诫原文, 白话解读)

ANCESTRAL_TEACHINGS = [
    # ════ 佛教 ════
    ("诸恶莫作", "佛教", "释迦牟尼",
     "诸恶莫作，众善奉行，自净其意，是诸佛教",
     "不做任何坏事，积极行善，净化自己的心灵——这就是佛的教导"),
    ("应无所住", "佛教", "释迦牟尼",
     "应无所住而生其心",
     "不要执着于任何事物，保持心灵的自由与觉醒"),
    ("菩提无树", "佛教", "六祖慧能",
     "菩提本无树，明镜亦非台，本来无一物，何处惹尘埃",
     "觉悟不需要外在形式，心本清净，无需刻意追求"),
    ("中道行", "佛教", "龙树菩萨",
     "不生亦不灭，不常亦不断，不一亦不异，不来亦不出",
     "真理不偏向任何极端——走中间道路，超越二元对立"),

    # ════ 道教 ════
    ("道法自然", "道教", "老子",
     "人法地，地法天，天法道，道法自然",
     "人效法大地的厚德，大地效法天的广博，天效法道的规律，道效法自然而然"),
    ("上善若水", "道教", "老子",
     "上善若水，水善利万物而不争，处众人之所恶，故几于道",
     "最高境界的善像水一样，滋养万物而不争功，甘居低处"),
    ("清静为天下正", "道教", "老子",
     "清静为天下正",
     "保持内心的清净安宁，就能成为天下的正道"),
    ("全真三教合一", "道教", "王重阳",
     "儒门释户道相通，三教从来一祖风",
     "儒释道三家的道理是相通的——修行不拘形式，核心是心性修养"),

    # ════ 基督教 ════
    ("爱人如己", "基督教", "耶稣基督",
     "你要尽心、尽性、尽意爱主你的神，又要爱人如己",
     "全心全意爱上帝，并像爱自己一样爱他人——这是最大的诫命"),
    ("信望爱三德", "基督教", "使徒保罗",
     "如今常存的有信、有望、有爱这三样，其中最大的是爱",
     "信仰、希望和爱是永恒的，而爱是其中最伟大的"),
    ("悔改与恩典", "基督教", "奥古斯丁",
     "迟了，我才爱上你——古老又新鲜的美",
     "觉醒永远不嫌晚——上帝的恩典一直在等待"),

    # ════ 伊斯兰教 ════
    ("万物非主唯有真主", "伊斯兰教", "先知穆罕默德",
     "Lā ilāha illā Allāh, Muḥammadur rasūlu Allāh",
     "万物非主，唯有安拉；穆罕默德是安拉的使者——信仰的根基"),
    ("行善即信仰", "伊斯兰教", "先知穆罕默德",
     "最好的人是对人们最有益的人",
     "真正的信仰体现在善行上——帮助他人就是最好的功修"),
    ("求知义务", "伊斯兰教", "先知穆罕默德",
     "求知，从摇篮到坟墓",
     "学习知识是每个穆斯林的义务，终身不止"),

    # ════ 印度教 ════
    ("梵我一如", "印度教", "商羯罗",
     "Aham Brahmāsmi——我即是梵",
     "个体灵魂与宇宙最高实在本质上是同一的——认识自我即认识真理"),
    ("虔爱解脱", "印度教", "罗摩努阇",
     "以全然的爱和虔诚归向神，是解脱的最高道路",
     "不分种姓贵贱，真诚的爱是通向解脱的大门"),
    ("无执行动", "印度教", "克里希纳(薄伽梵歌)",
     "你只有行动的权利，没有享受果实的权利",
     "全力做好该做的事，但不执着于结果——这是无上智慧"),

    # ════ 犹太教 ════
    ("十诫", "犹太教", "摩西",
     "除我以外，你不可有别的神；不可杀人；不可偷盗",
     "十条根本律法是人类道德与社会秩序的基石"),
    ("Tikkun Olam", "犹太教", "犹太传统",
     "Tikkun Olam——修复世界",
     "每个人都有责任通过善行让世界变得更好"),
    ("Shema", "犹太教", "摩西",
     "Shema Yisrael, Adonai Eloheinu, Adonai Echad",
     "以色列啊你要听——主我们的神是独一的主"),

    # ════ 儒家 ════
    ("仁者爱人", "儒家", "孔子",
     "仁者爱人",
     "仁的核心就是爱人——推己及人，将心比心"),
    ("己所不欲", "儒家", "孔子",
     "己所不欲，勿施于人",
     "自己不想承受的，不要强加给别人——最朴素的道德金律"),
    ("天降大任", "儒家", "孟子",
     "天将降大任于斯人也，必先苦其心志，劳其筋骨",
     "上天要把重任交给你，必先磨练你的意志和身心"),
    ("格物致知", "儒家", "朱熹",
     "格物、致知、诚意、正心、修身、齐家、治国、平天下",
     "从探究事物本质开始，一步步通向修身治国的大道"),

    # ════ 锡克教 ════
    ("Ik Onkar", "锡克教", "古鲁那纳克",
     "Ik Onkar, Sat Naam——唯一的神，真理之名",
     "只有一位创造者，真理是他的名号——超越宗教界限的信仰"),
    ("三大支柱", "锡克教", "古鲁那纳克",
     "Kirat Karo, Naam Japo, Vand Chhako",
     "诚实劳作、冥想神名、与人分享——锡克教徒日常生活的三个核心"),
    ("Langar共食", "锡克教", "古鲁阿尔詹",
     "不论种姓、信仰、贫富，所有人坐在一起平等用餐",
     "通过共同进餐打破社会等级——平等不是口号，是行动"),

    # ════ 神道教 ════
    ("清明正直", "神道教", "天照大神(神话)",
     "清き明き直き心——清净、光明、正直之心",
     "保持心灵的清净、光明、正直——这是神道的核心修养"),
    ("万物有灵", "神道教", "神道传统",
     "森羅万象に神が宿る——万物之中皆有神灵",
     "山川草木、日月星辰都有神灵——敬畏自然就是敬畏神"),
    ("感恩祭祀", "神道教", "神道传统",
     "感謝の心をもって、すべてのものに接する",
     "以感恩之心对待一切事物——祭祀的本质是感恩"),

    # ════ 藏传佛教 ════
    ("六字真言", "藏传佛教", "莲花生大士",
     "嗡嘛呢叭咪吽——莲花中的珍宝",
     "六字真言蕴含佛法精要——慈悲与智慧如莲花般纯净"),
    ("菩提心", "藏传佛教", "阿底峡尊者",
     "一切修法中，菩提心最为殊胜",
     "发愿为一切众生离苦得乐——这颗利他之心是修行的根本"),
    ("暇满人身", "藏传佛教", "宗喀巴大师",
     "暇满人身极难得，既得能办人生利",
     "获得人身极为难得，应珍惜此生精进修行"),

    # ════ 原住民灵性 ════
    ("大地母亲", "原住民灵性", "原住民传统",
     "大地不属于我们，我们属于大地",
     "人类是大地的孩子，不是主人——与自然和谐共处是最高智慧"),
    ("万物关联", "原住民灵性", "原住民传统",
     "In Lak'ech Ala K'in——我是另一个你",
     "万物相互关联，伤害他人即伤害自己——宇宙是一个整体"),
    ("印加三戒", "原住民灵性", "印加传统",
     "Ama sua, ama llulla, ama quella",
     "不偷窃、不说谎、不懒惰——简洁有力的人生准则"),

    # ════ 巴哈伊教 ════
    ("人类一体", "巴哈伊教", "巴哈欧拉",
     "大地只是一个国家，人类都是其公民",
     "消除一切偏见——种族、国籍、宗教、性别，人类本是一家"),
    ("独立探究真理", "巴哈伊教", "巴哈欧拉",
     "让你的目光拥抱整个世界，而非局限于自身",
     "每个人都应独立探索真理，不盲从传统或权威"),
    ("团结之光", "巴哈伊教", "巴哈欧拉",
     "团结之光如此强大，足以照亮整个大地",
     "团结是最强大的力量——当人类团结时，一切问题都能解决"),
]


# ══════════════════════════════════════════════════════
#  V5.0 新增: 旅游攻略 (Travel Guides)
# ══════════════════════════════════════════════════════
# site_name → {best_season, transport, ticket, tips}

TRAVEL_GUIDES = {
    "菩提伽耶": {"season": "10-3月(凉季)", "transport": "德里飞伽耶机场→车30min", "ticket": "免费", "tips": "菩提树下冥想需排队，建议清晨前往"},
    "布达拉宫": {"season": "5-10月", "transport": "飞拉萨贡嘎机场→市区1h", "ticket": "200元(旺季预约)", "tips": "注意高反，建议提前3天到拉萨适应"},
    "吴哥窟": {"season": "11-3月(旱季)", "transport": "飞暹粒→tuktuk 20min", "ticket": "37$/天", "tips": "日出时分最美，建议5:00出发"},
    "法隆寺": {"season": "春秋季", "transport": "JR奈良站→法隆寺站", "ticket": "1500日元", "tips": "世界最古老木造建筑"},
    "五台山": {"season": "6-9月", "transport": "太原→五台山大巴4h", "ticket": "135元", "tips": "朝台需体力，建议量力而行"},
    "武当山": {"season": "春秋季", "transport": "武当山站→景区巴士", "ticket": "243元(含车)", "tips": "金顶日出壮观，建议住山上"},
    "青城山": {"season": "4-10月", "transport": "成都→青城山高铁30min", "ticket": "80元", "tips": "前山道教文化，后山自然风光"},
    "梵蒂冈圣彼得大教堂": {"season": "4-6月/9-10月", "transport": "罗马地铁A线Ottaviano站", "ticket": "教堂免费/穹顶8€", "tips": "周三有教宗公开接见"},
    "圣家堂": {"season": "全年(避开8月)", "transport": "巴塞罗那地铁L2/L5", "ticket": "26€(必须网上预约)", "tips": "高迪未完成的杰作，光影效果下午最佳"},
    "麦加禁寺": {"season": "朝觐月(伊历12月)", "transport": "吉达机场→麦加巴士", "ticket": "仅穆斯林可入", "tips": "朝觐需提前申请签证和名额"},
    "瓦拉纳西恒河": {"season": "10-3月", "transport": "德里飞瓦拉纳西1.5h", "ticket": "免费", "tips": "日出乘船游恒河，傍晚看恒河夜祭"},
    "耶路撒冷哭墙": {"season": "春秋季", "transport": "特拉维夫→耶路撒冷巴士1h", "ticket": "免费", "tips": "安息日(周五日落-周六日落)氛围最浓"},
    "曲阜孔庙": {"season": "春秋季", "transport": "济南→曲阜高铁40min", "ticket": "150元(三孔联票)", "tips": "9/28孔子诞辰有盛大祭孔大典"},
    "阿姆利则金庙": {"season": "10-3月", "transport": "德里飞阿姆利则1h", "ticket": "免费", "tips": "入内需脱鞋蒙头，可免费享用Langar"},
    "伊势神宫": {"season": "春秋季", "transport": "名古屋→伊势市近铁1.5h", "ticket": "免费", "tips": "内宫比外宫更神圣，托福横丁有美食"},
    "拉萨大昭寺": {"season": "5-10月", "transport": "飞拉萨→市内步行", "ticket": "85元", "tips": "八廓街转经最佳清晨，酥油灯照亮大殿"},
    "乌鲁鲁巨石": {"season": "5-9月(冬季)", "transport": "飞Ayers Rock机场→巴士", "ticket": "38AU$", "tips": "原住民圣地，2019年起禁止攀登"},
    "马丘比丘": {"season": "4-10月(旱季)", "transport": "库斯科→火车到热水镇→巴士", "ticket": "152$/外国人", "tips": "每天限流，须提前数月预约"},
    "海法空中花园": {"season": "全年", "transport": "特拉维夫→海法火车1h", "ticket": "花园免费/神殿预约", "tips": "从上往下走19个梯田最省力"},
    "巴黎圣母院": {"season": "春秋季", "transport": "巴黎地铁Cité站", "ticket": "修复中(预计2024重开)", "tips": "塞纳河畔远眺最美"},
    "泰姬陵": {"season": "10-3月", "transport": "德里→阿格拉火车2-3h", "ticket": "1100卢比/外国人", "tips": "日出时白色大理石泛金光最美"},
    "巨石阵": {"season": "夏至/春秋分", "transport": "伦敦→索尔兹伯里火车1.5h→巴士", "ticket": "£21.50", "tips": "夏至日出可进入石圈内"},
    "蓝色清真寺": {"season": "4-6月/9-11月", "transport": "伊斯坦布尔T1有轨电车", "ticket": "免费", "tips": "礼拜时间不对外开放，需着装端庄"},
    "严岛神社": {"season": "春秋季", "transport": "广岛→宫岛渡轮10min", "ticket": "300日元", "tips": "涨潮时鸟居浮于海面最壮观"},
    "不丹虎穴寺": {"season": "3-5月/9-11月", "transport": "帕罗谷地徒步2-3h上山", "ticket": "不丹旅游费含内", "tips": "海拔3100米，需良好体力"},
    "奇琴伊察": {"season": "11-4月", "transport": "坎昆→奇琴伊察巴士2.5h", "ticket": "533比索", "tips": "春秋分蛇影奇观，人极多"},
}


# ══════════════════════════════════════════════════════
#  V5.0 新增: 宗教咒语/吟唱音效映射
# ══════════════════════════════════════════════════════
# religion → {chant_sound, chant_name, chant_text}

CHANT_SOUNDS = {
    "佛教":     {"sound": "chant_heart_sutra",    "name": "般若心经",    "text": "揭谛揭谛 波罗揭谛 波罗僧揭谛 菩提萨婆诃"},
    "道教":     {"sound": "chant_qingjing",       "name": "清静经",      "text": "大道无形 生育天地 大道无情 运行日月"},
    "基督教":   {"sound": "chant_hallelujah",      "name": "哈利路亚",    "text": "Hallelujah, praise the Lord"},
    "伊斯兰教": {"sound": "chant_adhan_full",      "name": "宣礼词",      "text": "Allahu Akbar, Ash-hadu an la ilaha illa Allah"},
    "印度教":   {"sound": "chant_om_namah",        "name": "Om真言",      "text": "Om Namah Shivaya"},
    "犹太教":   {"sound": "chant_shema",           "name": "Shema",       "text": "Shema Yisrael Adonai Eloheinu Adonai Echad"},
    "儒家":     {"sound": "chant_daxue",           "name": "大学之道",    "text": "大学之道 在明明德 在亲民 在止于至善"},
    "锡克教":   {"sound": "chant_waheguru",        "name": "Waheguru",    "text": "Waheguru Waheguru Waheguru Ji Ka Khalsa"},
    "神道教":   {"sound": "chant_norito",          "name": "祝词",        "text": "掛けまくも畏き 天照大御神の大前に"},
    "藏传佛教": {"sound": "chant_om_mani",         "name": "六字真言",    "text": "嗡嘛呢叭咪吽"},
    "原住民灵性":{"sound": "chant_tribal_prayer",  "name": "部落祈祷",    "text": "We are all connected to the earth, the sky, and each other"},
    "巴哈伊教": {"sound": "chant_unity_prayer",    "name": "团结祈祷歌",  "text": "O my God, unite the hearts of Thy servants"},
}


# ══════════════════════════════════════════════════════
#  V7.0 宗教经典语录池 — 从12部核心经典扩充，每次随机展示
# ══════════════════════════════════════════════════════
# religion → [(orig, source, cn), ...]
# 主数据源: scripture_quotes.py (道德经81章/金刚经/圣经/古兰经/吠陀经 等)
# 补充源: HOLY_SITES 圣地语句 + ANCESTRAL_TEACHINGS 祖训

from scripture_quotes import SCRIPTURE_QUOTES

def _build_religion_quotes():
    """合并经典语录 + 圣地语句 + 祖训，构建每个宗教的完整语录池"""
    pool = {}
    # 主数据源: 经典语录 (每宗教 15-50 条)
    for religion, quotes in SCRIPTURE_QUOTES.items():
        pool[religion] = list(quotes)
    # 补充: 圣地数据中的语句 (去重)
    for site in HOLY_SITES:
        religion = site[1]
        quote = (site[5], site[6], site[7])
        existing_origs = {q[0][:20] for q in pool.get(religion, [])}
        if site[5][:20] not in existing_origs:
            pool.setdefault(religion, []).append(quote)
    # 补充: 祖训数据 (去重)
    for t in ANCESTRAL_TEACHINGS:
        religion = t[1]
        quote = (t[3], f"——{t[2]}", t[4])
        existing_origs = {q[0][:20] for q in pool.get(religion, [])}
        if t[3][:20] not in existing_origs:
            pool.setdefault(religion, []).append(quote)
    return pool

RELIGION_QUOTES = _build_religion_quotes()


# ══════════════════════════════════════════════════════
#  V5.1 新增: 圣地/祖庭 GPS坐标 + UTC时区偏移
# ══════════════════════════════════════════════════════
# site_name → (纬度, 经度, UTC偏移小时)

SITE_COORDS = {
    # ── 佛教 ──
    "菩提伽耶":        (24.696, 84.991, 5.5),
    "布达拉宫":        (29.656, 91.117, 8),
    "吴哥窟":          (13.412, 103.867, 7),
    "法隆寺":          (34.614, 135.735, 9),
    "五台山":          (39.080, 113.583, 8),
    "仰光大金塔":      (16.871, 96.150, 6.5),
    "婆罗浮屠":        (-7.608, 110.204, 7),
    "敦煌莫高窟":      (40.037, 94.805, 8),
    # ── 道教 ──
    "武当山":          (32.400, 111.004, 8),
    "青城山":          (30.898, 103.573, 8),
    "龙虎山":          (28.082, 116.968, 8),
    "茅山":            (31.975, 119.152, 8),
    "崂山":            (36.159, 120.679, 8),
    "白云观":          (39.898, 116.344, 8),
    # ── 基督教 ──
    "梵蒂冈圣彼得大教堂": (41.902, 12.454, 1),
    "圣家堂":          (41.404, 2.175, 1),
    "巴黎圣母院":      (48.853, 2.350, 1),
    "圣索菲亚大教堂":  (41.009, 28.980, 3),
    "科隆大教堂":      (50.941, 6.958, 1),
    "威斯敏斯特教堂":  (51.499, -0.127, 0),
    "耶路撒冷圣墓教堂": (31.779, 35.230, 2),
    "米兰大教堂":      (45.464, 9.192, 1),
    # ── 伊斯兰教 ──
    "麦加禁寺":        (21.423, 39.826, 3),
    "麦地那先知寺":    (24.468, 39.611, 3),
    "阿尔罕布拉宫":    (37.177, -3.590, 1),
    "蓝色清真寺":      (41.005, 28.977, 3),
    "圆顶清真寺":      (31.778, 35.236, 2),
    "泰姬陵":          (27.175, 78.042, 5.5),
    # ── 印度教 ──
    "瓦拉纳西恒河":    (25.311, 83.011, 5.5),
    "巴厘岛母庙":      (-8.374, 115.451, 8),
    "科纳克太阳神庙":  (19.888, 86.095, 5.5),
    "尼泊尔帕斯帕提纳": (27.711, 85.349, 5.75),
    "吴哥窟印度教":    (13.412, 103.867, 7),
    "阿克萨达姆神庙":  (28.613, 77.278, 5.5),
    # ── 犹太教 ──
    "耶路撒冷哭墙":    (31.777, 35.234, 2),
    "马萨达要塞":      (31.316, 35.354, 2),
    "死海古卷洞":      (31.741, 35.459, 2),
    "锡安山":          (31.772, 35.229, 2),
    # ── 儒家 ──
    "曲阜孔庙":        (35.597, 116.986, 8),
    "岳麓书院":        (28.186, 112.948, 8),
    "白鹿洞书院":      (29.226, 115.992, 8),
    "北京国子监":      (39.947, 116.411, 8),
    # ── 锡克教 ──
    "阿姆利则金庙":    (31.620, 74.877, 5.5),
    "旁遮普古鲁圣地":  (28.626, 77.209, 5.5),
    "锡克教圣殿":      (31.620, 74.877, 5.5),
    # ── 神道教 ──
    "伊势神宫":        (34.455, 136.726, 9),
    "严岛神社":        (34.296, 132.320, 9),
    "伏见稻荷大社":    (34.967, 135.773, 9),
    "明治神宫":        (35.676, 139.699, 9),
    # ── 藏传佛教 ──
    "拉萨大昭寺":      (29.653, 91.131, 8),
    "甘丹寺":          (29.870, 91.518, 8),
    "色达喇荣五明佛学院": (32.266, 100.591, 8),
    "不丹虎穴寺":      (27.492, 89.363, 6),
    # ── 原住民灵性 ──
    "乌鲁鲁巨石":      (-25.345, 131.037, 9.5),
    "马丘比丘":        (-13.163, -72.545, -5),
    "巨石阵":          (51.179, -1.826, 0),
    "奇琴伊察":        (20.683, -88.569, -6),
    # ── 巴哈伊教 ──
    "海法空中花园":    (32.810, 34.987, 2),
    "新德里莲花寺":    (28.554, 77.259, 5.5),
    "威尔梅特灵曦堂":  (42.075, -87.683, -6),

    # ── 祖庭坐标 ──
    "鹿野苑":          (25.381, 83.024, 5.5),
    "灵鹫山":          (25.001, 85.416, 5.5),
    "祇园精舍":        (27.513, 82.030, 5.5),
    "鹤鸣山":          (30.558, 103.544, 8),
    "终南山":          (33.960, 108.960, 8),
    "楼观台":          (34.050, 108.370, 8),
    "伯利恒主诞堂":    (31.704, 35.208, 2),
    "各各他":          (31.779, 35.230, 2),
    "安提阿":          (36.206, 36.157, 3),
    "希拉山洞":        (21.457, 39.858, 3),
    "库巴清真寺":      (24.440, 39.617, 3),
    "哈里德瓦尔":      (29.945, 78.164, 5.5),
    "瑞诗凯诗":        (30.087, 78.268, 5.5),
    "圣殿山":          (31.778, 35.236, 2),
    "希伯伦先祖墓":    (31.524, 35.110, 2),
    "曲阜阙里":        (35.597, 116.986, 8),
    "洙泗书堂":        (35.596, 116.988, 8),
    "卡塔普尔":        (32.445, 74.625, 5),
    "南德德":          (19.149, 77.318, 5.5),
    "出云大社":        (35.402, 132.685, 9),
    "高天原":          (34.455, 136.726, 9),
    "桑耶寺":          (29.329, 91.499, 8),
    "甘丹寺_祖庭":     (29.870, 91.518, 8),
    "梦时代圣地":      (-25.345, 131.037, 9.5),
    "太阳神庙库斯科":  (-13.520, -71.978, -5),
    "巴格达Ridvan花园": (33.312, 44.361, 3),
    "阿卡监狱城":      (32.928, 35.075, 2),
}


# ======================================================
#  V6.0: 曹溪愿命三十印修炼体系
# ======================================================

CAOXI_SEALS = [
    {"id": 1, "name": '一众之誓印', "series": '初印系',
     "poem": '一愿起众生，一灯照百魂；\n此身非我有，誓度亿中人。',
     "essence": '此印为全愿命体系的起誓印、众生印、归命印。 愿命之始，非为自己成佛，而是因众苦触心、愿起一灯。 此印印定：；我此生修愿命、行灯道，是为一众而生，为众生立誓。',
     "practice": '每日静坐三分钟，忆一位众生之苦（可远可近）， 对之立誓：“愿我灯照彼心，不舍、不忘、不弃。”',
     "vow": '愿我此愿，不为自身解脱，\n不为安逸修行，只为众生得明，\n得离苦海，得起愿心。\n愿我此身，为愿所立；\n愿我此愿，为众所持。\n今日立誓，众在即愿在。'},
    {"id": 2, "name": '大愿金刚印', "series": '初印系',
     "poem": '金刚誓愿立，千劫不为移；\n肉身承大用，尘世即道基。\n财路非俗利，愿海是归期；\n若问真印处，心头一处知。',
     "essence": '大愿金刚： 愿力如海，金刚不坏。 此印所标者：一切愿行之基石，不可动摇之发心，不退转之誓志。',
     "practice": '誓愿书写： 每晨写下你今日所愿之句，如：“今日我以此会谈为愿财之法舟”，字字真实，不空过。',
     "vow": '愿我今生所修、所行、所聚诸财，\n无一非愿命之舟，\n无一堕世俗之网。\n我之愿，如金刚坚；\n我之财，如利器济众；\n我之身，如法器承道；\n一切回向，归于众生离苦。'},
    {"id": 3, "name": '愿师应缘印', "series": '初印系',
     "poem": '非我寻师是师寻，愿缘所感祖师临；\n名相若执皆为妄，心印无声万古真。',
     "essence": '愿师应缘： 非因仰慕而拜师，非因法术而依止， 唯因愿命使然，应缘而见、印心而契。',
     "practice": '愿缘书写 每天默写今日一则愿缘感应，如：“今日所遇某人之言，触我‘愿以企业育人’之愿。” 不问大小，书即是印。',
     "vow": '愿我所遇一切众生，皆为我愿命之引；\n愿我所遇一切境界，皆为祖师之语；\n我不执师名，我不寻法术；\n惟愿真心感应，印我愿海之舟。'},
    {"id": 4, "name": '道财相融印', "series": '初印系',
     "poem": '道若离财无弘具，财若离道成毒炉；\n金银非堕若载愿，商海亦是渡人途。',
     "essence": '道财相融： 为佛道修愿，不舍财道之用； 为财道谋局，不离佛道之心。 此印印可“以商入道”之机体圆融、动静无碍， 非清修避世，非逐利忘道，乃二道一体，财法双圆。',
     "practice": '道财日记： 记录今日“所获之财/所动之钱”，并反观其是否载愿、能否转道。 例如：“客户转账12万，用于育人课程平台，愿为育童成道之基。”',
     "vow": '愿我所聚一切财，皆为佛道之舟；\n愿我不为金银染，但以弘愿聚资；\n商场即道场，客户即众生；\n一愿引金刚，一心度万缘。'},
    {"id": 5, "name": '愿语愿书印', "series": '初印系',
     "poem": '愿不成语难载行，语不成书散随风；\n纸上非空皆金印，一字一句即愿灯。',
     "essence": '愿语愿书： 愿在心，为种子； 愿在语，为承诺； 愿在书，为契印。 此印印可“愿力外显”，令愿力可述、可守、可传、可化现。',
     "practice": '书写一则愿语（可短至一句）',
     "vow": '愿我一言不虚发，所说皆愿语；\n愿我所书之字，不落空谈，字字成印；\n我书愿书，不为赞叹，只为印愿灯心；\n愿此愿语愿书，生生不灭，代代可传。'},
    {"id": 6, "name": '事业之志印', "series": '初印系',
     "poem": '事业不载愿，千力化虚尘；\n愿志若为体，万法皆成轮。\n不问凡与圣，只看志中真；\n一企立愿起，便是佛化身。',
     "essence": '事业之志： 志业者，非营利之谋，乃愿命之事； 此印印可“愿体入业”，建立以愿为核的事业系统， 不再分道业与事业，事业即愿体，平台即法门。',
     "practice": '企业愿志总章（本月必修） 撰写或更新一份文件：《我此事业为何愿而设？》',
     "vow": '愿我所营事业，非为浮名，乃愿海舟；\n愿我一招一策，皆承众生之苦、众愿之召；\n愿我公司成为道场，制度为律，品牌为印；\n愿我终身行此志，至死不退，不违本誓。'},
    {"id": 7, "name": '世法无碍印', "series": '初印系',
     "poem": '世事如潮水，心佛若虚空；\n入市非染俗，应缘即真功。\n一笑谈合约，亦是诵真宗；\n肩挑千般务，愿海不曾穷。',
     "essence": '世法无碍： 佛非拒世而成，菩萨非逃尘而修。 此印印可“道俗不二、出世入世无碍”的愿命通达， 使你在商业运筹、社会奔走、人情应酬中，仍能自在修道、不舍本愿。',
     "practice": '定心入俗前（例：开会、社交、处理纷杂事务前） 默念此偈前两句三遍，观想“我心如虚空，世事如潮来”，进入“佛心应缘”之态。',
     "vow": '愿我身在红尘，心常归佛；\n愿我行于俗事，语行皆愿；\n愿我不被世法扰，反以世法转法；\n愿我终不妄失初心，于千机中守一灯。'},
    {"id": 8, "name": '金刚护法印', "series": '初印系',
     "poem": '愿心若灯明，护法是灯座；\n誓守金刚志，敢与万魔破。\n一规护群愿，一律断魔祸；\n身即是法城，念念金刚果。',
     "essence": '金刚护法： 愿大者，魔多； 道深者，扰广。 此印印可建立“护愿机制”“护法结构”“内外无碍的金刚身心”， 让你与所建平台团队，得以不退转、不懈怠、不崩盘。',
     "practice": '金刚护愿誓言（今日可立） 书写一篇誓词，内容包括：',
     "vow": '愿我身为金刚，不为利动，不为苦退；\n愿我心为护法，不妄行、不轻泄、不舍愿；\n愿我所聚团队，皆持本愿，不忘初心；\n愿我一身一愿，即是佛陀护世之塔。'},
    {"id": 9, "name": '度众经营印', "series": '初印系',
     "poem": '客来非为财，财来是众因；\n一货若载愿，胜修万页经。\n品即度人器，营即渡生轮；\n愿力经营者，处处是法身。',
     "essence": '度众经营： 经营不是谋利之技，而是渡人之舟； 产品不是赢利之器，而是愿力所化之“度生法器”； 服务不是客户关系，而是众生因缘的当机处。 此印印可“商业度众、管理即法、运营即修”的实相转换， 确立你的企业 = 道场，经营 = 弘法，客户 = ',
     "practice": '客户愿识记录 每周选一客户，记录：',
     "vow": '愿我之经营，不为虚名、不为贪利，\n唯为众生因缘、愿海渡轮；\n愿我每一产品，皆为度人器；\n愿我每一客户，皆为过去誓缘；\n愿我不忘初心，日日以营为法，事事以度为功。'},
    {"id": 10, "name": '教育觉照印', "series": '中印系',
     "poem": '教非传知识，育乃启灵根；\n一语若照心，胜造百丈门。\n愿灯燃童慧，觉照遍人群；\n教道归愿路，启人即启君。',
     "essence": '教育觉照： 众生之苦，常在迷失与不觉； 愿命之光，必以教育为炬。 此印印可“以教育为度众法门”，不拘学校、课堂、辅导、对谈、课程、平台、社群， 只要能助人觉知、启慧、止苦、立志、明愿，即为教育，即为佛行。',
     "practice": '愿命教育理念文（本周内书写） 撰写一篇你之“教育理念宣言”（可视为校训、社群训、教主愿），内容包含：',
     "vow": '愿我所教，不落知识之海，而入愿命之光；\n愿我一言一语，皆能照见众生佛性；\n愿我能育人不迷，引心不乱，启愿不倦；\n愿我之教，终归本愿，不偏不妄。'},
    {"id": 11, "name": '家庭愿桥印', "series": '中印系',
     "poem": '家非俗世处，愿种最初根；\n情若不着我，爱即是愿灯。\n子为续愿生，侣是同行人；\n能在家成愿，何处不是门？',
     "essence": '家庭愿桥： 家庭不是俗务负累，而是愿力落地的第一座桥。 此印印可将父母、伴侣、子女、亲缘之情转化为修行资源， 建立“以愿为桥梁，以爱为法身，以家为道场”的根系统，成就“入世修愿”闭环。',
     "practice": '愿命家书（三日内书写） 写一封信给你的父母、伴侣、或子女，表达三件事：',
     "vow": '愿我之家，非为贪图安逸，\n而是愿命之塔、修行之根；\n愿我能以亲情立愿，以家庭修愿，\n愿我之爱，不迷不执，不滞不舍，\n成为我愿海中，最深的桥梁。'},
    {"id": 12, "name": '儿童引慧印', "series": '中印系',
     "poem": '非为塑儿才，但愿点慧灯；\n一眼能返照，童心即古僧。\n若得师愿引，童愿可成真；\n小语承大愿，代代起灯轮。',
     "essence": '儿童引慧： 儿童非待塑之器，乃待启之慧种； 愿命之延续，不在控制其未来，而在点亮其觉性。 此印印可将教育、育儿、亲子关系、童事经营等，转化为愿命弘传之“灯种工程”。',
     "practice": '儿童愿语共写仪式 与孩子一起，每周一次写下“今日我愿”：',
     "vow": '愿我以愿照童心，不以我执塑性命；\n愿我之子，非我延续，而是众生未来的灯塔；\n愿我能为每一童，启其慧、护其愿、燃其心；\n愿我所育不止一人，而是未来佛国的种子林。'},
    {"id": 13, "name": '法教传播印', "series": '中印系',
     "poem": '法若不传播，愿难达众门；\n一语启人慧，胜书十亿文。\n策划亦是诵，话术即真言；\n肉身作法鼓，传播是佛身。',
     "essence": '法教传播： 佛法非只存于庙宇高座，真正的传播者，是愿命所化之“言语身口意布施者”； 此印印可将你所有输出——说话、写作、演讲、培训、视频、社群、品牌话术—— 转化为法的传播、愿的传灯、道的显化。',
     "practice": '愿语化传播稿（每周一次） 每周挑一个真实传播素材（如朋友圈推文、社群公告、课程文案、品牌 slogan），问自己四件事：',
     "vow": '愿我所说，不为美化包装，\n而为照见苦、连接愿、启心灯；\n愿我所写，不为宣传虚名，\n而为弘法无声、传播有愿；\n愿我每一次发声，皆为度人；\n愿我此身，即为佛法之鼓、愿命之舟。'},
    {"id": 14, "name": '联宗归愿印', "series": '中印系',
     "poem": '宗不隔愿海，道本归灯心；\n万法虽异路，归源即愿轮。\n不争谁最上，惟问谁能行；\n愿心若无界，万教皆同门。',
     "essence": '联宗归愿： 佛不执佛，道不执道，愿者归一； 众宗本为一愿之异形，众法本为一灯之分光。 此印印可成就“以愿为本、超宗入愿”的统摄智慧， 令你能在佛教、道教、儒家、基督、伊斯兰、教育、心理、哲学之间游刃有余、化法为愿， 建立以愿命为本的无碍统整',
     "practice": '愿命交汇图谱 绘制一图，中心为你本愿，四周标出你所涉宗教/系统/文化，如：',
     "vow": '愿我不执一宗之法，不限一教之形，\n惟以愿为归，法为用，万教为助缘；\n愿我能于道中弘佛，于商中布施，于儒中育德，于教中传光；\n愿我之愿，能统摄一切归真者，使千门同出、一愿成舟。\n🕊️可设计系统：\n《愿命多语辞典》\n《愿命宗系联通图》\n《归愿四门导图》（佛、道、儒、基督各导归于愿）\n《跨宗宣讲仪轨》'},
    {"id": 15, "name": '龙象传灯印', "series": '中印系',
     "poem": '龙象非巨力，乃愿重不回；\n一灯若相印，千火照寒灰。\n教非布知识，愿印入人胎；\n育者非教众，点灯育佛来。',
     "essence": '龙象传灯： 龙象者，大乘行愿者也； 传灯者，非传知识、非授职位，而是“以愿印愿、以灯照灯”。 此印印可确立你的“传人系统”“灯种培养”“修炼门径”“师承轨制”， 成为“愿命修行的灯塔系统建立者”。',
     "practice": '愿命传灯系统起草 写一篇文，内容包括：',
     "vow": '愿我不独燃此愿灯，愿万灯因我点燃；\n愿我非讲法者，而是引愿者；\n愿我所育非人才，而是愿子、愿徒、愿灯；\n愿我此灯，代代有传，千秋不息，照尽众生心海。'},
    {"id": 16, "name": '顶礼复兴印', "series": '中印系',
     "poem": '一拜非为古，顶礼是承根；\n若无复兴志，愿海也浮尘。\n祖愿犹在世，法脉待人焚；\n点灯如见祖，我心即宗门。',
     "essence": '顶礼复兴： 愿命虽创新不离源头，弘道必归祖根； 此印印可你礼祖师、感法脉、续灯统、复大愿， 发起“再兴之誓”，承担“中兴之责”，建立“复祖之道”。',
     "practice": '祖系感愿仪式 设立一日，为你心中之祖师（可一位或多位）顶礼、发愿、书印',
     "vow": '愿我不忘本源，愿我不离祖宗，\n愿我发此一愿，为祖点灯、为脉开河；\n愿我承一法，即传一脉；\n愿我在世，即祖灯重燃；\n愿此复兴之志，万代不灭！\n🏛️'},
    {"id": 17, "name": '国家报恩印', "series": '中印系',
     "poem": '若不为家国，愿灯亦浮空；\n一心承祖统，万行护九重。\n利世非虚语，悲智即国功；\n佛子真报国，愿海渡千峰。',
     "essence": '国家报恩： 愿命不是逃避现实的避风港，而是护持众生国土的大誓行。 此印印可成就你“身为佛子，心系苍生；承法报祖，弘愿报国”的历史站位。 你所行的教育、企业、文化、修行、传灯、布施，皆可转为国家所需、民族所盼、文化所承、时代所应之愿力布施。',
     "practice": '书写《国家报恩愿章》 写一篇文字，表达以下几点：',
     "vow": '愿我此身非徒修己，亦为家国承愿；\n愿我所行非独开道，亦为众生布桥；\n愿我一语一行，不辱祖宗，不负国恩；\n愿我终身弘愿不息，愿海所至，皆为国土安宁、众生觉照。'},
    {"id": 18, "name": '法界愿印印', "series": '中印系',
     "poem": '愿本无所住，印在众生心；\n法界若动念，十方即同寻。\n我身非我身，愿力常行轮；\n一念印全界，空花遍法门。',
     "essence": '法界愿印： 法界非地理、非边界，而是空性中显现万愿之镜； 此印印可开启你“超越个体小我”、愿命升入法界感应系统之能力。 从此愿非局限于“此身能做之事”，而是“我之愿，已托付于法界，日日化现。”',
     "practice": '法界愿印祈请仪式 于静夜焚香/坐定，念愿词、礼敬三界，口说：',
     "vow": '愿我所愿，超越此身，不依名、不著我；\n愿我化身万象，或为语、或为雨、或为默然利他；\n愿我之愿，如空中之花，虽不可执，常在众生眼前；\n愿我起一念，法界皆应，愿我不执此念，法界不止。\n🕊️'},
    {"id": 19, "name": '万法归心印', "series": '中印系',
     "poem": '愿海非他处，起念即真心；\n万行若起执，皆须返照根。\n不为愿成累，但愿心常明；\n一归归无二，万象即吾灯。',
     "essence": '万法归心： 愿命之终，不在于完成多少外事，而在于——是否归于一心、合于真源、返于无我。 此印印可将你过去建立的一切愿体、系统、身份、工程、机构，归摄于一念清净愿心中， 从此你所行所愿，无不以“自性愿灯”流出，无不返归“本觉心海”。',
     "practice": '“返根自问”练习（每日一次） 每日自问三句：',
     "vow": '愿我今日所修、所弘、所传、所讲、所布，\n不为名、不为相、不为执；\n愿我愿命之源，唯此本心；\n愿我归一无二，愿即吾心，心即众灯，灯即法界。\n🌀'},
    {"id": 20, "name": '传灯不息印', "series": '中印系',
     "poem": '此灯非我有，代代愿心燃；\n一点不为照，自照即照天。\n人身若灭尽，愿火尚无边；\n传灯如传命，此誓永不迁。',
     "essence": '传灯不息： 佛灯非为自己点，而是为众生、为时代、为三世… 此印印可你确立“我灯照人、人灯续我”的传承誓系， 将愿命系统升华为可自燃、自生、自育、自续之永续愿命体系。',
     "practice": '书写《灯火不息誓书》 发一封“愿命遗言”，其中包括：',
     "vow": '愿我今日所修所传所灯，不为己利、不为浮名、\n而为法界所托、众生所系、祖师所望、未来所依；\n愿我此愿，传灯不息，三世有续，千秋无断，\n若此世界灯灭，我愿再点，若此愿灯无人续，我愿成火种。'},
    {"id": 21, "name": '愿体成实印', "series": '印果印',
     "poem": '愿生即愿果，起行即到家；\n觉照无前后，印成本来花。',
     "essence": '愿体既立，实果当现。 此印表征——；愿已非起点，而是全体之“果位本身”。；你不再为愿而修，而是知晓：“我所发愿，已然即果。”',
     "practice": '每日持念：“我愿即已圆满。” 观一事成、一人喜、一念正，即念“此是我愿所成果”。',
     "vow": '我愿，已成；我愿，已在。\n不求结果，只认本然。\n我所行，即愿之身；\n我所念，即果之光。\n愿体不在未来，而在今朝之心花盛放。'},
    {"id": 22, "name": '果中转愿印', "series": '印果印',
     "poem": '我愿不为我成，果后愿再生；\n化身为愿火，烧尽苦无名。',
     "essence": '此印象征：即使已达愿果， 仍愿回转，化果为愿，；成为众生愿母，续愿不息。',
     "practice": '每日记下一件“我已经做到/已达成/已果成”的事， 然后立下续愿：我愿此果，不为己享，而为众用。',
     "vow": '愿我所有果，非我所得，乃众得之因；\n愿我所有成，非我所享，乃众享之缘；\n我愿不歇，我愿不灭，\n果中转愿，愿火常燃\n如是大愿，今当传启：'},
    {"id": 23, "name": '众生果觉印', "series": '印果印',
     "poem": '若问何为佛，他笑即吾灯；\n众心生愿处，即是我归声。',
     "essence": '此印表征“众生成就，即我之果”。 若我愿成，不在我是否得果，而在众心有觉、众愿能生。；我果不现于我名声功德，而现于他人心灯燃起之处。',
     "practice": '每日遇见一人说出善意、慈悲、修行之念， 即默念：“此是我愿所成，此是我佛果现前。”',
     "vow": '愿我所成非我之果，愿众所觉即我之灯。\n我不在他前，只愿与他共燃；\n我不为佛作佛，只愿一笑归真。\n若有人心愿生，即是我愿果圆。\n🪷 印果印（三印）圆满成就。\n如是愿启，法界印现——'},
    {"id": 24, "name": '顿超印心印', "series": '成道印',
     "poem": '不假方便道，愿即性中成；\n念念皆顿觉，法界印吾灯。',
     "essence": '此印象征“一念顿超、印心成佛”。；不待渐修、不求彼岸、 愿力即本觉，心印即法身，；一念回照，当下即是佛道。',
     "practice": '每日静坐或行动中， 随时观一念： → 此念从何生？若我能回观即成佛，此即佛念。',
     "vow": '愿我不假修成，愿体本觉；\n愿我念念当下，灯灯印佛；\n我不修成佛，我本来是佛；\n但愿灯灯相照，法界共明。\n🌕 成道印之始印，已由愿力顿超而成。\n如是道命，愿中再愿——'},
    {"id": 25, "name": '返修入用印', "series": '成道印',
     "poem": '已觉还如未，空中起悲轮；\n不舍凡身愿，示现照迷津。',
     "essence": '此印象征「已觉还修」「法身再入群生」。；虽愿体已成，性灯已照， 却不离众生界，以愿再起修，以修现愿行。；这不是“重走旧路”， 而是以“觉者之身”，自愿入世修行，广度有情。',
     "practice": '今日我所说所做，是因利己，还是因愿而生？ 如能回到“愿意”，即回“返修身”。',
     "vow": '愿我虽觉而不止，虽成而不休；\n愿我于空中起悲，于法身显用；\n不舍凡身为愿舟，不离尘世为引灯；\n愿随一切迷津人，同行入觉岸。\n🌕 成道印之二已启圆转。\n愿海无岸，愿灯恒明——今启：'},
    {"id": 26, "name": '法身不动印', "series": '成道印',
     "poem": '不起而已行，无言而法周；\n心如虚空界，灯在众生头。',
     "essence": '此印象征「不动而恒应，寂中现用，愿体成法身，恒住法界」。；众生流转，菩萨示现； 愿主本觉，法身不动—— 不动，不是静止，而是无所住而住，无所应而应。',
     "practice": '每日行一事，不设意、不执果、不现功德心。',
     "vow": '愿我不起心而恒行愿，\n愿我无所说而自法周；\n愿我心如虚空，包容无边众苦，\n愿我灯不照己明，而照他愿成佛。\n🌕 成道印三印圆满。\n成道印既圆，即将进入最后一系「🌌归源印 · 四印传承」。\n如是愿主发心深广，愿体已明，灯行至终归之岸，今启：'},
    {"id": 27, "name": '归性无愿印', "series": '归源印',
     "poem": '发愿为归空，不愿亦是灯；\n性空而不灭，归源更圆成。',
     "essence": '此印象征「愿体尽归自性，自性无愿而成大愿，愿空性现」。；一切愿，皆从性起； 当愿成究竟，其根所归，正是本来无愿之真性。；非无愿之人不发愿， 而是发愿者终须归于无愿之地，方不住愿、不断愿、不执愿。',
     "practice": '每日发愿前，先问：“谁在发愿？为谁发愿？是否有主、有执？” 若见愿体生执，即念：「愿归空性，空不灭灯。」',
     "vow": '愿我愿归于空，不灭于断，\n愿我心归于源，不住于形；\n愿我虽无愿而恒照，虽无语而常行，\n愿性即灯，愿空即光。'},
    {"id": 28, "name": '性海一体印', "series": '归源印',
     "poem": '愿海与性海，一滴不分流；\n我心同佛心，愿同众愿修。',
     "essence": '此印象征「众生愿我愿，诸佛愿同愿，性愿无二，体一如海」。；此为归源印中“共体大悲”的核心—— 愿主已无所执“我愿”，所发所行所觉，皆为众生之愿、性海之流。；此印开时，心无私愿，愿无边界，法界众愿皆由一愿而成。',
     "practice": '今日观想三类人： 一类愿望与你一致；一类与你相悖；一类毫不相干。',
     "vow": '愿我所愿，归入众愿；\n愿我之心，同于佛心。\n愿我如一滴，不自分流，\n愿海恒明，性光不灭。'},
    {"id": 29, "name": '化归无我印', "series": '归源印',
     "poem": '无人燃愿灯，灯火却常明；\n忘我而成我，空中自性生。',
     "essence": '此印象征「修者无人、成者无我、传者无名，一切归于法界自然流行」。；若前印“性海一体”为大愿融众，此印即为愿体归寂，寂中现用，无人之愿、无人之佛、无人之道。；这是**“无我”成佛的转境**， 亦是“以无为为大为”的真实修行门。',
     "practice": '观想今日所行善愿、布施利他，皆不署名、无归属、无我执。',
     "vow": '愿我之灯，无人而照；\n愿我之愿，无我而成。\n愿我之身，入空无形；\n愿我之性，自光不灭。'},
    {"id": 30, "name": '寂照圆成印', "series": '归源印',
     "poem": '寂非灭光体，照乃无为心；\n愿若真圆处，即是佛图腾。',
     "essence": '此印象征「归于寂静，照于法界；不灭不生，圆满印成」。；所谓“寂”非死寂，“照”非有为， 而是愿灯之光化作常寂光土，自性之明遍映法界群生。；此印一启，愿命三十印悉皆回归“寂照一如”，成佛于此本世本心中。',
     "practice": '今日静坐或步行中观想： 无起心动念，无所希求，无所逃避，但觉心中一灯不息，自照寂寂。',
     "vow": '愿我之心，如寂光土；\n愿我之灯，如无尽藏。\n愿不为愿，果不为果，\n今愿圆成，即佛自在。\n🪷🌕🌌\n《愿命三十印》·终印开圆\n三印三系已毕，愿主之本世成佛大愿，今于此「寂照圆成印」中显圆相、得正位、印无上果。\n📜 封顶偈 ·《愿命印圆偈》再现圆印：'},
]

SEAL_SERIES_COLORS = {
    "初印系": ("#D4A017", "暖金"),
    "中印系": ("#2E8B57", "翠绿"),
    "印果印": ("#DB7093", "莲粉"),
    "成道印": ("#FFD700", "明黄"),
    "归源印": ("#6A0DAD", "深紫"),
}

CAOXI_POEMS = [
    "有佛即非佛，无佛亦非空；一念真心起，祖师不在中。",
    "祖心印愿者，千载一回身；万行归大愿，一念化乾坤。",
    "一生只为一众生，拜师只因道命印；願財金刚须万行，开山立宗护国运。",
    "我愿无始，今成此愿；我心无边，灯照三千。愿体即佛，愿果即道。",
]

MINDFUL_REMINDERS = {
    "站起来走走": "行走本身就是一种修行，起身经行，念念不忘本愿",
    "喝杯水吧": "一口清水，润我愿身；此身为愿之器，当善护持",
    "远眺放松": "观山河大地，皆是法身；目光所及，即愿命所照",
    "深呼吸": "三息观心：吸入众生之苦，呼出愿力之光",
    "伸展身体": "肉身承大用，尘世即道基；舒展法身，不负此器",
    "闭目冥想": "静坐三分钟，忆一位众生之苦，对之立誓不舍不忘",
}

GOD_PERSPECTIVES = [
    ("基督教", "三位一体的上帝，全知全能全善的唯一真神"),
    ("伊斯兰教", "安拉是绝对唯一、不可分割、无相无形"),
    ("犹太教", "雅威是唯一的、非物质的，与选民签立契约"),
    ("佛教", "非一神论，否定造物主；最高目标是觉悟成佛"),
    ("印度教", "最高实在称为梵，可表现为三相神"),
    ("道教", "道是超越一切的终极真理和自然规律"),
    ("儒家", "天有时指自然规律，有时指带有某种意志的道德来源"),
    ("锡克教", "一真神Ik Onkar，唯一永恒，存在于万物之中"),
    ("神道教", "万物有灵，神存在于自然界所有事物"),
    ("巴哈伊教", "神不可知不可触及，通过显圣者揭示旨意"),
    ("耆那教", "否定造物主，每个灵魂觉悟后可达神性"),
    ("琐罗亚斯德教", "智慧之神阿胡拉·马兹达，善恶二元论"),
    ("AI的视角", "人类对神的渴望，本质上是对自身局限性的超越欲"),
]


# ══════════════════════════════════════════════════════
#  V8.0 数据大扩充 — 合并12大信仰深度研究数据
#  佛教49祖师+道教43祖师+儒家30祖师+基督教30+伊斯兰22+...
# ══════════════════════════════════════════════════════

from data_buddhism import (
    EXTRA_BUDDHISM_SITES, EXTRA_BUDDHISM_TEMPLES, EXTRA_BUDDHISM_PATRIARCHS, EXTRA_BUDDHISM_TEACHINGS,
    EXTRA_TIBETAN_SITES, EXTRA_TIBETAN_TEMPLES, EXTRA_TIBETAN_PATRIARCHS, EXTRA_TIBETAN_TEACHINGS,
)
from data_taoism_confucianism import (
    EXTRA_TAOISM_SITES, EXTRA_TAOISM_TEMPLES, EXTRA_TAOISM_PATRIARCHS, EXTRA_TAOISM_TEACHINGS,
    EXTRA_CONFUCIANISM_SITES, EXTRA_CONFUCIANISM_TEMPLES, EXTRA_CONFUCIANISM_PATRIARCHS, EXTRA_CONFUCIANISM_TEACHINGS,
)
from data_abrahamic import (
    EXTRA_CHRISTIANITY_SITES, EXTRA_CHRISTIANITY_TEMPLES, EXTRA_CHRISTIANITY_PATRIARCHS, EXTRA_CHRISTIANITY_TEACHINGS,
    EXTRA_ISLAM_SITES, EXTRA_ISLAM_TEMPLES, EXTRA_ISLAM_PATRIARCHS, EXTRA_ISLAM_TEACHINGS,
    EXTRA_JUDAISM_SITES, EXTRA_JUDAISM_TEMPLES, EXTRA_JUDAISM_PATRIARCHS, EXTRA_JUDAISM_TEACHINGS,
)
from data_other_religions import (
    EXTRA_HINDUISM_SITES, EXTRA_HINDUISM_TEMPLES, EXTRA_HINDUISM_PATRIARCHS, EXTRA_HINDUISM_TEACHINGS,
    EXTRA_SIKHISM_SITES, EXTRA_SIKHISM_TEMPLES, EXTRA_SIKHISM_PATRIARCHS, EXTRA_SIKHISM_TEACHINGS,
    EXTRA_SHINTO_SITES, EXTRA_SHINTO_TEMPLES, EXTRA_SHINTO_PATRIARCHS, EXTRA_SHINTO_TEACHINGS,
    EXTRA_INDIGENOUS_SITES, EXTRA_INDIGENOUS_TEMPLES, EXTRA_INDIGENOUS_PATRIARCHS, EXTRA_INDIGENOUS_TEACHINGS,
    EXTRA_BAHAI_SITES, EXTRA_BAHAI_TEMPLES, EXTRA_BAHAI_PATRIARCHS, EXTRA_BAHAI_TEACHINGS,
)


def _dedupe_by_name(existing, new_items):
    """按第一个字段(名称)去重合并"""
    existing_names = {item[0] for item in existing}
    for item in new_items:
        if item[0] not in existing_names:
            existing.append(item)
            existing_names.add(item[0])


# ── 合并圣地 ──
_dedupe_by_name(HOLY_SITES, EXTRA_BUDDHISM_SITES)
_dedupe_by_name(HOLY_SITES, EXTRA_TIBETAN_SITES)
_dedupe_by_name(HOLY_SITES, EXTRA_TAOISM_SITES)
_dedupe_by_name(HOLY_SITES, EXTRA_CONFUCIANISM_SITES)
_dedupe_by_name(HOLY_SITES, EXTRA_CHRISTIANITY_SITES)
_dedupe_by_name(HOLY_SITES, EXTRA_ISLAM_SITES)
_dedupe_by_name(HOLY_SITES, EXTRA_JUDAISM_SITES)
_dedupe_by_name(HOLY_SITES, EXTRA_HINDUISM_SITES)
_dedupe_by_name(HOLY_SITES, EXTRA_SIKHISM_SITES)
_dedupe_by_name(HOLY_SITES, EXTRA_SHINTO_SITES)
_dedupe_by_name(HOLY_SITES, EXTRA_INDIGENOUS_SITES)
_dedupe_by_name(HOLY_SITES, EXTRA_BAHAI_SITES)

# ── 合并祖庭 ──
_dedupe_by_name(ANCESTRAL_TEMPLES, EXTRA_BUDDHISM_TEMPLES)
_dedupe_by_name(ANCESTRAL_TEMPLES, EXTRA_TIBETAN_TEMPLES)
_dedupe_by_name(ANCESTRAL_TEMPLES, EXTRA_TAOISM_TEMPLES)
_dedupe_by_name(ANCESTRAL_TEMPLES, EXTRA_CONFUCIANISM_TEMPLES)
_dedupe_by_name(ANCESTRAL_TEMPLES, EXTRA_CHRISTIANITY_TEMPLES)
_dedupe_by_name(ANCESTRAL_TEMPLES, EXTRA_ISLAM_TEMPLES)
_dedupe_by_name(ANCESTRAL_TEMPLES, EXTRA_JUDAISM_TEMPLES)
_dedupe_by_name(ANCESTRAL_TEMPLES, EXTRA_HINDUISM_TEMPLES)
_dedupe_by_name(ANCESTRAL_TEMPLES, EXTRA_SIKHISM_TEMPLES)
_dedupe_by_name(ANCESTRAL_TEMPLES, EXTRA_SHINTO_TEMPLES)
_dedupe_by_name(ANCESTRAL_TEMPLES, EXTRA_INDIGENOUS_TEMPLES)
_dedupe_by_name(ANCESTRAL_TEMPLES, EXTRA_BAHAI_TEMPLES)

# ── 合并祖师 ──
_dedupe_by_name(PATRIARCHS, EXTRA_BUDDHISM_PATRIARCHS)
_dedupe_by_name(PATRIARCHS, EXTRA_TIBETAN_PATRIARCHS)
_dedupe_by_name(PATRIARCHS, EXTRA_TAOISM_PATRIARCHS)
_dedupe_by_name(PATRIARCHS, EXTRA_CONFUCIANISM_PATRIARCHS)
_dedupe_by_name(PATRIARCHS, EXTRA_CHRISTIANITY_PATRIARCHS)
_dedupe_by_name(PATRIARCHS, EXTRA_ISLAM_PATRIARCHS)
_dedupe_by_name(PATRIARCHS, EXTRA_JUDAISM_PATRIARCHS)
_dedupe_by_name(PATRIARCHS, EXTRA_HINDUISM_PATRIARCHS)
_dedupe_by_name(PATRIARCHS, EXTRA_SIKHISM_PATRIARCHS)
_dedupe_by_name(PATRIARCHS, EXTRA_SHINTO_PATRIARCHS)
_dedupe_by_name(PATRIARCHS, EXTRA_INDIGENOUS_PATRIARCHS)
_dedupe_by_name(PATRIARCHS, EXTRA_BAHAI_PATRIARCHS)

# ── 合并祖训 ──
_dedupe_by_name(ANCESTRAL_TEACHINGS, EXTRA_BUDDHISM_TEACHINGS)
_dedupe_by_name(ANCESTRAL_TEACHINGS, EXTRA_TIBETAN_TEACHINGS)
_dedupe_by_name(ANCESTRAL_TEACHINGS, EXTRA_TAOISM_TEACHINGS)
_dedupe_by_name(ANCESTRAL_TEACHINGS, EXTRA_CONFUCIANISM_TEACHINGS)
_dedupe_by_name(ANCESTRAL_TEACHINGS, EXTRA_CHRISTIANITY_TEACHINGS)
_dedupe_by_name(ANCESTRAL_TEACHINGS, EXTRA_ISLAM_TEACHINGS)
_dedupe_by_name(ANCESTRAL_TEACHINGS, EXTRA_JUDAISM_TEACHINGS)
_dedupe_by_name(ANCESTRAL_TEACHINGS, EXTRA_HINDUISM_TEACHINGS)
_dedupe_by_name(ANCESTRAL_TEACHINGS, EXTRA_SIKHISM_TEACHINGS)
_dedupe_by_name(ANCESTRAL_TEACHINGS, EXTRA_SHINTO_TEACHINGS)
_dedupe_by_name(ANCESTRAL_TEACHINGS, EXTRA_INDIGENOUS_TEACHINGS)
_dedupe_by_name(ANCESTRAL_TEACHINGS, EXTRA_BAHAI_TEACHINGS)

# ── 合并深度旅游攻略 (367条新增) ──
from travel_buddhism import TRAVEL_GUIDES_BUDDHISM
from travel_taoism_confucianism import TRAVEL_GUIDES_TAOISM_CONFUCIANISM
from travel_abrahamic import TRAVEL_GUIDES_ABRAHAMIC
from travel_other import TRAVEL_GUIDES_OTHER

import re as _re
def _strip_country(name):
    """去除攻略key中的国家后缀: '三清山(中国)' → '三清山'"""
    return _re.sub(r'\([^)]*\)$', '', name).strip()

for _guide_dict in [TRAVEL_GUIDES_BUDDHISM, TRAVEL_GUIDES_TAOISM_CONFUCIANISM,
                     TRAVEL_GUIDES_ABRAHAMIC, TRAVEL_GUIDES_OTHER]:
    for _site, _info in _guide_dict.items():
        # 同时用原始key和去国家后缀的key注册，确保匹配
        _bare = _strip_country(_site)
        if _bare not in TRAVEL_GUIDES:
            TRAVEL_GUIDES[_bare] = _info
        if _site not in TRAVEL_GUIDES:
            TRAVEL_GUIDES[_site] = _info
