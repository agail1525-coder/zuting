# ============================================================================
# ZUTING Heartbeat Daemon v4.2 — CEO++业务驱动智能建设引擎
# ============================================================================
# 核心机制: SCAN -> BUILD(并行) -> VERIFY -> PUSH -> 循环
# v4.0: 自愈状态/质量门禁/配额守卫/价值评分/失败模式/App归属commit/丰富Dashboard
# v4.1: 经验沉淀/BLG三问硬门禁/BUILD构建硬条件
# v4.2: CEO++最高业务驱动注入 — 对标全球TOP10旅行平台/GOGOGO七律/页面++PRD标准
#        Scan+Build双端均注入战略目标+竞品对标+差异化护城河+视觉标准+人工保护
#        SCP最高宪法协议: PRD先行铁律/开发协议规则库/CLAUDE.md持续进化
#        竞品功能全吸收令: 探索发现10大网站全部功能并植入JOINUS.COM
...
# --- 最高宪法协议 SCP (注入Scan+Build双端) ---

$CEOBusinessRules = @'

[最高宪法协议 (SCP) — 所有人类工程师和AI智能体必须无条件遵守]
...
主动探索发现以下10大旅行网站的全部优势功能，逐一植入JOINUS.COM:
  1. Booking.com  — 目的地详情/评价体系/预订流程/价格展示/搜索筛选/地图
  2. Trip.com     — UI风格(白底蓝主色#0066FF)/大图卡片/搜索/套餐/攻略/社区/会员
  3. Airbnb       — 沉浸式图片/体验产品/地图交互/社区感/Wishlist/分享
  4. Agoda        — 亚洲本地化/优惠促销/移动端/闪购/积分
  5. TripAdvisor  — UGC评价/目的地攻略/排行榜/社区/问答/照片墙
  6. AmEx Travel  — 高端定位/会员体系/精选推荐/品质感/专属优惠
  7. Expedia      — 套餐打包/交叉销售/行程管理/价格日历/比价
  8. Skyscanner   — 搜索比价UI/数据可视化/极简设计/价格提醒/灵活日期
  9. Kayak        — 筛选器/多维比较/数据密集展示/价格追踪
  10. Priceline   — 促销机制/限时优惠/用户激励/Express Deals
每发现一个竞品优势功能，必须评估可行性并纳入PRD规划。

=== SCP-03 PRD先行铁律 — 无PRD不动代码 ===
任何功能模块/子页面/延伸页面的开发/重构/升级，必须:
  STEP 1: 深度调研 — 打开对应竞品页面，记录功能点、交互细节、UI模式
  STEP 2: 撰写PRD — 在 docs/prd/ 下创建/更新对应PRD文档
           主模块: docs/prd/M{XX}-PRD-{模块名}.md
           子页面: docs/prd/P{XX}-PRD-{页面名}.md
           延伸功能: docs/prd/F{XX}-PRD-{功能名}.md
  STEP 3: PRD必须包含: 竞品对标分析/用户故事/页面线框/API契约/验收标准
  STEP 4: PRD通过后才允许开始代码和UI/UX开发
  STEP 5: 开发完成后对照PRD逐项验收
粒度: 一个PRD = 一个页面的详细设计，绝不允许一个PRD涵盖多页面

=== SCP-04 开发协议规则库 ===
路径: docs/protocols/
  00-最高宪法协议.md | 01-UI设计规范.md | 02-API契约规范.md
  03-组件开发规范.md | 04-数据模型规范.md | 05-安全规范.md
  06-国际化规范.md | 07-Git与部署规范.md | 08-PRD撰写规范.md
  09-竞品分析规范.md | 10-AI智能体协作规范.md
所有工程师和智能体开发前必须阅读相关协议。协议持续更新。

=== SCP-05 CLAUDE.md持续进化令 ===
CLAUDE.md = 全项目最高配置文件(宪法级)
- 新铁律/规范/最佳实践 -> 评估后写入CLAUDE.md
- 重大版本迭代后 -> 审查并更新CLAUDE.md
- 禁止降级: 已有铁律只能升级或标注废弃，不可静默删除

=== 页面对标映射 ===
  圣地详情 = Booking目的地页(大图轮播+评分+位置+实用信息+附近推荐)
  祖庭详情 = Airbnb体验页(沉浸式图片+故事叙述+地图+评价)
  祖师详情 = TripAdvisor景点页(概述+历史+评价+相关推荐)
  行程详情 = Expedia行程页(日程表+地图+费用明细+预订状态)
  首页 = Trip.com首页(白底蓝主色+大图卡片+搜索+分类导航)
  列表页 = Booking/Kayak(多维筛选+即时结果+地图切换)

=== 差异化护城河 (宗教文化深度) ===
  经文/教义/历史/传承 -> 10大平台都没有的垂直内容
  朝圣路线规划 -> 文化意义+修行指导 > 普通旅行路线
  三十印+修行成就 -> 独特游戏化体验 > 普通打卡
  12信仰x60圣地x27祖庭 -> 全球最全祖庭数据库

=== CEO++ 执行铁律 (GOGOGO七律) ===
  [GG-R01] tsc门禁 — 完成后必须 pnpm tsc --noEmit 通过
  [GG-R02] 文件互斥 — 同一文件不能被两个Agent同时修改
  [GG-R03] 最小变更原则 — 只修目标，不顺手重构
  [GG-R04] 结构化变更报告 — 输出变更摘要
  [GG-R05] 波次执行 — W1安全修复 -> W2接口变更 -> W3跨模块
  [GG-R06] import链检测 — 改export时追踪所有import方
  [GG-R07] 回滚预案 — 大改前git tag

=== 视觉标准 ===
  白色背景 + 蓝色主色 #0066FF (Trip.com风格)
  大图卡片、沉浸式Hero图、高质量图片
  微阴影、圆角、呼吸感留白
  图片本地化(public/images/)，禁止依赖外部CDN

=== AI功能标准 ===
  小鸿AI聊天不需要登录(只有修行模块需要登录)
  专精宗教文化+朝圣路线推荐(不是通用旅行助手)

=== 人工修改保护 (HE铁律) ===
  [HE-01] 人工修改 > AI判断。CEO发起/指示/批准的 = 人工修改
  [HE-04] 绝对禁止覆盖/还原/重写人工修改
  [HE-06] 冲突时优先级: 人工 > PRD > AI
'@
...
[CEO++ 战略使命]
JOINUS.COM 的目标是成为全球祖庭文化旅行平台 NO.1。
所有任务发现必须服务于这个战略目标。对标全球TOP10旅行平台:
Booking.com(详情页/评价/预订) | Trip.com(UI风格/搜索) | Airbnb(沉浸式/社区)
Agoda(亚洲本地化/移动端) | TripAdvisor(UGC/攻略) | AmEx Travel(高端/会员)
Expedia(套餐/行程) | Skyscanner(搜索/极简) | Kayak(筛选/比较) | Priceline(促销/激励)

任务发现优先级必须结合竞品差距:
- 我们的页面是否达到Booking/Airbnb/Trip.com同类页面的体验水平?
- 我们是否充分发挥了宗教文化垂直领域的差异化优势(12信仰/60圣地/27祖庭/28祖师/39祖训/30印)?
- 用户旅程是否完整: 发现->探索->规划->预订->体验->评价->分享?

[SCP-03 PRD先行铁律 — 无PRD不动代码]
页面/功能任务必须先检查 docs/prd/ 是否有对应PRD文档:
- 如果没有PRD: 任务类型应为"撰写PRD"(如 "web-prd-favorites-page"), 优先级提升
- 如果有PRD但过时: 任务类型应为"更新PRD"
- 如果有PRD且最新: 才允许输出代码开发/重构任务
PRD命名: M{XX}-PRD-{模块}.md | P{XX}-PRD-{页面}.md | F{XX}-PRD-{功能}.md

[SCP-04 开发协议规则库维护]
检查 docs/protocols/ 目录, 发现缺失的协议文档时输出创建任务:
  00-最高宪法协议 | 01-UI设计规范 | 02-API契约规范 | 03-组件开发规范
  04-数据模型规范 | 05-安全规范 | 06-国际化规范 | 07-Git与部署规范
  08-PRD撰写规范 | 09-竞品分析规范 | 10-AI智能体协作规范
...
D. 对每个端(API/Web/Admin/Mobile/小程序)执行全审++(六维审查):
   ...
   - V6-竞品对标: 每个页面对标TOP10旅行平台同类页面，找出体验差距和缺失功能
   - 找出: 可以新增的功能页面(用户收藏/评论/推荐/社交分享/搜索/会员/攻略)
E. 按优先级排序: P0(崩溃/安全) > P1(功能缺失/竞品差距大) > P2(体验优化) > P3(新功能/差异化)
F. 页面视觉标准: 白底+蓝主色#0066FF、大图卡片、沉浸式Hero、微阴影圆角、呼吸感留白
...
- description中必须注明要遵守的铁律编号(如R-01, HH-V03, BLG-02, GG-R01, SCP-03)
- description中应注明对标的竞品页面(如"对标Booking目的地详情页")
- 页面任务必须符合页面++标准: 垂直切片(HH-V01~V06) + 六维审查 + 竞品对标
- SCP-03 PRD先行: 页面/功能任务必须先有PRD，无PRD则先输出PRD撰写任务
- PRD任务id: {app}-prd-{target}, 如 "web-prd-favorites-page", "web-prd-search-system"
- 协议任务id: docs-protocol-{name}, 如 "docs-protocol-ui-design"
...
[最高宪法协议 SCP — 最高业务驱动]
战略目标: JOINUS.COM = 全球祖庭文化旅行平台 NO.1
- SCP-02: 集10大旅行网站全部功能之大成 + 宗教文化垂直深度
- 对标: Booking/Trip.com/Airbnb/Agoda/TripAdvisor/AmEx/Expedia/Skyscanner/Kayak/Priceline
- 页面对标: 圣地=Booking目的地页 | 祖庭=Airbnb体验页 | 祖师=TripAdvisor景点页 | 行程=Expedia行程页
- 视觉标准: 白底+蓝主色#0066FF、大图卡片、沉浸式Hero、微阴影圆角、呼吸感留白
- 差异化护城河: 宗教文化深度(经文/教义/历史/传承) > 通用旅行平台
- 图片本地化: 使用public/images/，禁止依赖外部CDN
- GOGOGO七律: GG-R01(tsc门禁) GG-R03(最小变更) GG-R06(import链检测)
- 人工修改保护: HE-01/HE-04/HE-06，CEO修改绝对不可覆盖

[SCP-03 PRD先行铁律]
- 开发/重构页面或功能前，必须先检查 docs/prd/ 下是否有对应PRD
- 如果任务是PRD撰写类: 调研竞品 -> 写PRD(含竞品对标/用户故事/线框/API契约/验收标准)
- 如果任务是代码开发类: 先读取对应PRD，严格按PRD执行，完成后逐项验收
- PRD路径: docs/prd/M{XX}-PRD-{模块}.md | P{XX}-PRD-{页面}.md | F{XX}-PRD-{功能}.md

[SCP-04 开发协议规则库]
- 遵循 docs/protocols/ 下的规范文档
- 发现新的可复用规范时，建议写入协议库
...
[页面++ PRD驱动标准]
- 新增/重构页面前先查看 docs/prd/ 对应PRD文档
- 垂直切片: 页面→组件→API调用→Controller→Service→Prisma→DB 缺一不可(HH-V01)
- 空壳零容忍: Alert占位/mock数据/断联组件 → 完善或删除(HH-V02)
- 交付≠编译通过: 必须有真实API调用+真实数据渲染+交互可用(HH-V03)
...
function Show-ExitSummary {
    ...
    Write-Host "    ZUTING Daemon v4.2 — Exit Summary" -ForegroundColor Yellow
    Write-Host "  ============================================" -ForegroundColor Cyan
...
function Start-HeartbeatLoop {
    Write-Host ""
    Write-Host "  ============================================" -ForegroundColor Cyan
    Write-Host "    ZUTING Heartbeat Daemon v4.2" -ForegroundColor Yellow
    Write-Host "    CEO++ Business-Driven | TOP10 Benchmark" -ForegroundColor Yellow
    Write-Host "    SCAN -> BUILD -> VERIFY -> PUSH -> LOOP" -ForegroundColor DarkYellow
    Write-Host "  ============================================" -ForegroundColor Cyan
    Write-Host ""
    ...
    while ($true) {
        ...
        while ($taskIndex -lt $sortedTasks.Count -or $RunningJobs.Count -gt 0) {
            ...
            while ($freeSlots -gt 0 -and $taskIndex -lt $sortedTasks.Count) {
                ...
                $buildPrompt = "你是ZUTING平台的D1建设Agent。请执行以下任务，遵守CLAUDE.md铁律和CEO++最高业务驱动。" + "`n" + $CEOBusinessRules + "`n" + $humanContext + $expContext + "`n任务: $($task.name)`n`n$($task.description)" + "`n" + $IronRulesInjection + $verifySuffix
