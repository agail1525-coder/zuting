"""
部署 Religion 商业实践深度案例到线上:
1. 上传 schema.prisma → prisma db push (加 businessCases Json?)
2. 解析种子数据 → 写入 DB
3. 刷 Redis 缓存
"""
import paramiko
...
LOCAL_CASES = "E:/ZUTING/services/api/prisma/seed-religion-business-cases.ts"
...
# Step 4: Parse seed-religion-business-cases.ts and create JS seed script
print("\n=== 4. Prepare seed script ===")
with open(LOCAL_CASES, 'r', encoding='utf-8') as f:
    ts_content = f.read()

m = re.search(r'export const RELIGION_BUSINESS_CASES[^{]*=\s*(\{.*\n\});', ts_content, re.DOTALL)
if not m:
    print("  FAILED to parse RELIGION_BUSINESS_CASES")
    exit(1)
...
const RELIGION_BUSINESS_CASES = {obj_literal};

async function main() {{
  let n = 0;
  for (const [slug, cases] of Object.entries(RELIGION_BUSINESS_CASES)) {{
  ...
  console.log(`Total: ${{n}} religions updated with businessCases`);
}}

main().catch(e => {{ console.error(e); process.exit(1); }}).finally(() => prisma.$disconnect());
"""

seed_path = f'{api_dir}/seed-religion-business-cases.js'
...
# Step 5: Run seed
print("\n=== 5. Execute seed ===")
run(f'cd {api_dir} && node seed-religion-business-cases.js')
...
bc = d.get("businessCases")
if bc:
    print("  businessCases.cases:", len(bc.get("cases", [])), "cases")
    print("  businessCases.masterQuotes:", len(bc.get("masterQuotes", [])), "quotes")
    print("  businessCases.practices:", len(bc.get("practices", [])), "practices")
    print("  businessCases.research:", len(bc.get("research", [])), "research")
    print("  businessCases.books:", len(bc.get("books", [])), "books")
else:
    print("  businessCases: MISSING!")
'''
with sftp.open('/tmp/verify_biz_cases.py', 'w') as f:
    f.write(verify_py)
run('python3 /tmp/verify_biz_cases.py')
run(f'rm -f /tmp/verify_biz_cases.py {seed_path}', show=False)
